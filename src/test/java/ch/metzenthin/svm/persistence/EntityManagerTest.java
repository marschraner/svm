 ...
 * EntityManager interface = persistence manager API cheap, doesn't obtain a JDBC Connection
 * EntityManagerFactory.createEntityManager() starts its persistence context interact with and use
 * EntityManager interface for a single unit of work in a single thread scope:
 * EntityManagerFactory.createEntityManager() ... EntityManager.close() EntityTransaction:
 * Everything between begin() and commit() occurs in one transaction commit(): performs dirty
 * checking of the persistence context and synchronizes with the database EntityManager.flush():
 * forces dirty checking synchronization manually. Any time during a transaction. Identifier
 * strategy (siehe Kapitel 4.2.5): SVM: AUTO_INCREMENT (siehe DDLs) d.h. nach Insert wird der
 * Identifier generiert bzw. hochgezählt. Nach EntityManager.persist(...) macht Hibernate einen
 * Insert und der Identifier ist danach vorhanden Bevorzugt gemäss Buch (siehe Kapitel 11.3.2) wäre:
 * pre-insert. Dann führt Hibernate den Insert so spät wie möglich aus. Bei
 * EntityManager.persist(...) wird nur der Identifier generiert (ohne Insert)
 *
 * <p>Fehlerhaftes Verhalten von EntityManager: - Ausgangslage: Erzeugen von zwei EM-Instanzen. Mit
 * der ersten Instanz: modifizieren einer Entität, Commit. Danach Lesen der modifizierten Instanz
 * mit der zweiten Instanz. - Symptom: Die Änderung der ersten Instanz ist in der zweiten Instanz
 * nicht immer sichtbar. - Das fehlerhafte Verhalten liegt nicht an der Verwendung des built-in
 * connection pools (siehe Warn-Meldung unten). Es tritt mit älteren Versionen von Hibernate auf
 * (getestet mit 4.3.9 und 5.0.1). Mit einer neueren Version von Hibernate (5.2.9) ist das
 * fehlerhafte Verhalten weg. - Änderungen sind schon nach clear und erneutem Lesen sichtbar -
 * Refresh verhält sich jetzt auch wie erwartet und wie im Buch Java Persistence with Hibernate
 * beschrieben. Warnung sichtbar durch: log4j.logger.org.hibernate=INFO WARN
 * DriverManagerConnectionProviderImpl:93 - HHH000402: Using Hibernate built-in connection pool (not
 * for production use!) Lösung: Verwendung eines anderen Connection Pools, z.B. c3p0 oder Bitronix
 * (wie im Buch Java Persistence with Hibernate)
 *
 * @author Hans Stamm
 */
...
public class EntityManagerTest {
  ...
  /**
   * Unsynchronized mode, reading data in auto-commit mode, nontransactional: No flushing (or
   * commit) allowed
   */
  @Test
  ...
  public void testVisibilityOfModificationInSecondEntityManager_afterCloseOfA() {
    ...
    /* Folgendes Fehlverhalten tritt ab Hibernate 5.2.9. nicht mehr auf (daher auskommentiert):
            ...
            entityManagerFactory = createEntityManagerFactory();
    */
    // mit einem neuen EntityManager von der neuen Factory. Die Änderung ist erwartungsgemäss
    // sichtbar.
    emB = entityManagerFactory.createEntityManager();
    schuelerCodeB = getInitialSchuelerCode(emB);
    assertEquals(beschreibungNew, schuelerCodeB.getBeschreibung());
  ...
  public void testVisibilityOfModificationInSecondEntityManager_afterCloseOfA_synchronize() {
    ...
    // mit neuem EntityManager, der auch den richtigen Wert liefert, dank
  ...
  public void testVisibilityOfModificationInSecondEntityManager_createEmBAfterCommit() {
    ...
    EntityManager emB = entityManagerFactory.createEntityManager();
    // emB sieht Änderung von emA bereits nach commit von emA, wenn emB erst nach dem commit erzeugt
    // wird
    SchuelerCode schuelerCodeB = getInitialSchuelerCode(emB);
    assertEquals(beschreibungNew, schuelerCodeB.getBeschreibung());
  ...
  public void testVisibilityOfModificationInSecondEntityManager_refresh() {
    ...
    // Dies widerspricht dem Kapitel 10.2.6. Refreshing data auf Seite 241 in Java Persistence with
  ...
  private SchuelerCode getSchuelerCodeByKuerzel(EntityManager em, String kuerzel) {
    ...
    throw new RuntimeException(
        "One result expected but found "
            + resultList.size()
            + " looking up SchuelerCode with Kuerzel "
            + kuerzel);
  }

  private List<SchuelerCode> getSchuelerCodesByKuerzel(EntityManager em, String kuerzel) {
    TypedQuery<SchuelerCode> typedQuery =
        em.createQuery("select c from SchuelerCode c where kuerzel = :kuerzel", SchuelerCode.class);
