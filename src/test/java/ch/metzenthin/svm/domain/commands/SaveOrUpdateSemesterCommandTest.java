 ...
 */
class SaveOrUpdateSemesterCommandTest {
  ...
  void testExecute() {
    ...
    assertFalse(
        checkIfSemesterAvailable(
            "1912/1913",
            Semesterbezeichnung.ERSTES_SEMESTER,
            new GregorianCalendar(1912, Calendar.AUGUST, 22),
            new GregorianCalendar(1913, Calendar.FEBRUARY, 22),
    ...
    assertEquals(
        Semesterbezeichnung.ERSTES_SEMESTER, semestersSaved.get(2).getSemesterbezeichnung());
    assertEquals("1912/1913", semestersSaved.get(1).getSchuljahr());
    assertEquals(
        Semesterbezeichnung.ERSTES_SEMESTER, semestersSaved.get(1).getSemesterbezeichnung());
    assertEquals("1912/1913", semestersSaved.get(0).getSchuljahr());
    assertEquals(
        Semesterbezeichnung.ZWEITES_SEMESTER, semestersSaved.get(0).getSemesterbezeichnung());

    // Semester bearbeiten (neuer Semesterbeginn, neues Semesterende)
    Semester semester1Modif =
        new Semester(
            "1912/1913",
            Semesterbezeichnung.ZWEITES_SEMESTER,
            new GregorianCalendar(1913, Calendar.FEBRUARY, 27),
            new GregorianCalendar(1913, Calendar.JULY, 17),
            new GregorianCalendar(1912, Calendar.APRIL, 25),
            new GregorianCalendar(1912, Calendar.MAY, 7),
            null,
            null);
    saveOrUpdateSemesterCommand =
        new SaveOrUpdateSemesterCommand(semester1Modif, semester1, semestersSaved);
    commandInvoker.executeCommandAsTransaction(saveOrUpdateSemesterCommand);
    assertEquals(3, semestersSaved.size());
    assertFalse(
        checkIfSemesterAvailable(
  ...
  private boolean checkIfSemesterAvailable(
      String schuljahr,
    ...
    for (Semester semester : semestersAll) {
      if (semester.getSchuljahr().equals(schuljahr)
          && semester.getSemesterbezeichnung().equals(semesterbezeichnung)
          && semester.getSemesterbeginn().equals(semesterbeginn)
          && semester.getSemesterende().equals(semesterende)
          && semester.getFerienbeginn1().equals(ferienbeginn1)
          && semester.getFerienende1().equals(ferienende1)
          && ((semester.getFerienbeginn2() == null && ferienbeginn2 == null)
              || (semester.getFerienbeginn2() != null
                  && semester.getFerienbeginn2().equals(ferienbeginn2)))
          && ((semester.getFerienende2() == null && ferienende2 == null)
              || (semester.getFerienende2() != null
                  && semester.getFerienende2().equals(ferienende2)))) {
        return true;
      }
    }
    return false;
