 ...
 */
class RemoveDispensationFromSchuelerCommandTest {
  ...
  void testExecute() {
    ...
    Dispensation dispensation1 =
        new Dispensation(
            new GregorianCalendar(2012, Calendar.JANUARY, 15),
            new GregorianCalendar(2012, Calendar.MARCH, 31),
            null,
            "Zu klein");
    AddDispensationToSchuelerAndSaveCommand addDispensationToSchuelerAndSaveCommand =
        new AddDispensationToSchuelerAndSaveCommand(dispensation1, null, savedSchueler);
    ...
    Dispensation dispensation2 =
        new Dispensation(
            ...
            new GregorianCalendar(2013, Calendar.MARCH, 31),
            null,
            "Beinbruch");
    addDispensationToSchuelerAndSaveCommand =
        new AddDispensationToSchuelerAndSaveCommand(dispensation2, null, savedSchueler);
    commandInvoker.executeCommandAsTransaction(addDispensationToSchuelerAndSaveCommand);

    Dispensation dispensation3 =
        new Dispensation(
            ...
            "Armbruch");
    addDispensationToSchuelerAndSaveCommand =
        new AddDispensationToSchuelerAndSaveCommand(dispensation3, null, savedSchueler);
    commandInvoker.executeCommandAsTransaction(addDispensationToSchuelerAndSaveCommand);
    ...
    RemoveDispensationFromSchuelerCommand removeDispensationFromSchuelerCommand =
        new RemoveDispensationFromSchuelerCommand(1, savedSchueler);
    ...
    if (neusteZuoberst) {
      assertEquals(
          "Armbruch", updatedSchueler.getDispensationen().get(0).getGrund()); // neuste zuoberst
      assertEquals("Zu klein", updatedSchueler.getDispensationen().get(1).getGrund());
    } else {
      assertEquals(
          "Armbruch", updatedSchueler.getDispensationen().get(1).getGrund()); // neuste zuletzt
