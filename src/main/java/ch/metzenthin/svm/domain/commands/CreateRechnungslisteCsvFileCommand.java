...
public class CreateRechnungslisteCsvFileCommand extends CreateListeCommand {
  ...
  public CreateRechnungslisteCsvFileCommand(
      List<? extends Semesterrechnung> semesterrechnungList,
  ...
  public void execute() {
    ...
    try {
      ...
      for (Semesterrechnung semesterrechnung : semesterrechnungList) {
        ...
        boolean showVorrechnung =
            semesterrechnung.getRechnungsdatumVorrechnung() != null
                || (semesterrechnung.getWochenbetragVorrechnung() != null
                    && semesterrechnung.getWochenbetragVorrechnung().compareTo(BigDecimal.ZERO)
                        != 0)
                || (semesterrechnung.getErmaessigungVorrechnung() != null
                    && semesterrechnung.getErmaessigungVorrechnung().compareTo(BigDecimal.ZERO)
                        != 0)
        ...
        out.write(separator);
        BigDecimal ermaessigungStipendiumVorrechnung =
            semesterrechnung.getErmaessigungStipendiumVorrechnung();
        if (showVorrechnung
            && ermaessigungStipendiumVorrechnung != null
            && ermaessigungStipendiumVorrechnung.compareTo(BigDecimal.ZERO) != 0) {
        ...
        out.write(separator);
        if (semesterrechnung.getErmaessigungNachrechnung() != null
            && semesterrechnung.getErmaessigungNachrechnung().compareTo(BigDecimal.ZERO) != 0) {
          out.write(semesterrechnung.getErmaessigungNachrechnung().toString());
        }
        out.write(separator);
        if (semesterrechnung.getZuschlagNachrechnung() != null
            && semesterrechnung.getZuschlagNachrechnung().compareTo(BigDecimal.ZERO) != 0) {
          out.write(semesterrechnung.getZuschlagNachrechnung().toString());
        }
        out.write(separator);
        BigDecimal ermaessigungStipendiumNachrechnung =
            semesterrechnung.getErmaessigungStipendiumNachrechnung();
        if (ermaessigungStipendiumNachrechnung != null
            && ermaessigungStipendiumNachrechnung.compareTo(BigDecimal.ZERO) != 0) {
        ...
        if (checkNotEmpty(semesterrechnung.getBemerkungen())) {
          out.write(
              semesterrechnung.getBemerkungenLineBreaksReplacedByCommaOrPeriod().replace(";", ","));
        }
        out.write(separator);
        String schuelerAsStr =
            semesterrechnung.getAktiveSchuelerRechnungsempfaengerAsStr(previousSemester);
