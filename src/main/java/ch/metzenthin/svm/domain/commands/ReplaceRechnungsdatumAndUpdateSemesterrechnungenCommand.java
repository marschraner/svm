...
public class ReplaceRechnungsdatumAndUpdateSemesterrechnungenCommand implements Command {
  ...
  public ReplaceRechnungsdatumAndUpdateSemesterrechnungenCommand(
      List<Semesterrechnung> semesterrechnungen,
      Rechnungstyp rechnungstyp,
      Calendar rechnungsdatum) {
  ...
  public void execute() {
    ...
    for (Semesterrechnung semesterrechnung : semesterrechnungen) {
      ...
      if (rechnungstyp == Rechnungstyp.VORRECHNUNG) {
        ...
        // Das Kopieren der Zahlungen geschieht nur, wenn die entsprechende Zahlung in der
        // Nachrechnung noch
        // nicht gesetzt ist.
        if (semesterrechnung.getBetragZahlung1Nachrechnung() == null
            && semesterrechnung.getBetragZahlung1Vorrechnung() != null) {
          semesterrechnung.setBetragZahlung1Nachrechnung(
              semesterrechnung.getBetragZahlung1Vorrechnung());
        }
        if (semesterrechnung.getDatumZahlung1Nachrechnung() == null
            && semesterrechnung.getDatumZahlung1Vorrechnung() != null) {
          semesterrechnung.setDatumZahlung1Nachrechnung(
              semesterrechnung.getDatumZahlung1Vorrechnung());
        }
        if (semesterrechnung.getBetragZahlung2Nachrechnung() == null
            && semesterrechnung.getBetragZahlung2Vorrechnung() != null) {
          semesterrechnung.setBetragZahlung2Nachrechnung(
              semesterrechnung.getBetragZahlung2Vorrechnung());
        }
        if (semesterrechnung.getDatumZahlung2Nachrechnung() == null
            && semesterrechnung.getDatumZahlung2Vorrechnung() != null) {
          semesterrechnung.setDatumZahlung2Nachrechnung(
              semesterrechnung.getDatumZahlung2Vorrechnung());
        }
        if (semesterrechnung.getBetragZahlung3Nachrechnung() == null
            && semesterrechnung.getBetragZahlung3Vorrechnung() != null) {
          semesterrechnung.setBetragZahlung3Nachrechnung(
              semesterrechnung.getBetragZahlung3Vorrechnung());
        }
        if (semesterrechnung.getDatumZahlung3Nachrechnung() == null
            && semesterrechnung.getDatumZahlung3Vorrechnung() != null) {
          semesterrechnung.setDatumZahlung3Nachrechnung(
              semesterrechnung.getDatumZahlung3Vorrechnung());
        }
      }
