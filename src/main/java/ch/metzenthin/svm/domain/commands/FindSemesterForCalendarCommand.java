...
public class FindSemesterForCalendarCommand implements Command {
  ...
  public void execute() {
    ...
    for (int i = 0; i < erfassteSemester.size(); i++) {
      if ((erfassteSemester.get(i).getSemesterbeginn().before(calendar)
              || erfassteSemester.get(i).getSemesterbeginn().equals(calendar))
          && (erfassteSemester.get(i).getSemesterende().after(calendar)
              || erfassteSemester.get(i).getSemesterende().equals(calendar))) {
        ...
        return;
      } else if (i + 1 < erfassteSemester.size()
          && erfassteSemester.get(i + 1).getSemesterende().before(calendar)
          && erfassteSemester.get(i).getSemesterbeginn().after(calendar)) {
