...
public class CalculateMonatsstatistikKurseCommand implements Command {
  ...
  public void execute() {
    ...
    Semester previousSemesterBeforeSemesterbeginn =
        checkIfMonatContainsSemesterbeginnAndGetPreviousSemester();
    if (previousSemesterBeforeSemesterbeginn != null && !schuelerIdsAnzahlAnmeldungen.isEmpty()) {
      // Implizite Abmeldungen nur berücksichtigen, falls bereits Schüler fürs nächste Semester
      // erfasst wurden
  ...
  private void calculateAnzahlLektionen(EntityManager entityManager) {
    ...
    Query query =
        entityManager.createQuery(
            "select count(k) from Kursanmeldung k where k.kurs.semester.semesterId = :semesterId and"
  ...
  private void calculateAnzahlAnmeldungenSchueler(EntityManager entityManager) {
    TypedQuery<Object[]> typedQuery =
        entityManager.createQuery(
            "select k.schueler.personId, count(k) from Kursanmeldung k where"
                + " (k.anmeldedatum >= :statistikMonatBeginn and k.anmeldedatum <= :statistikMonatEnde) group by k.schueler.personId",
            Object[].class);
  ...
  private void calculateAnzahlAbmeldungenSchueler(EntityManager entityManager) {
    TypedQuery<Object[]> typedQuery =
        entityManager.createQuery(
            "select k.schueler.personId, count(k) from Kursanmeldung k where"
                + " (k.abmeldedatum >= :statistikMonatBeginn and k.abmeldedatum <= :statistikMonatEnde) group by k.schueler.personId",
            Object[].class);
    typedQuery.setParameter(STATISTIK_MONAT_BEGINN, statistikMonatBeginn);
    typedQuery.setParameter(STATISTIK_MONAT_ENDE, statistikMonatEnde);
    for (Object[] result : typedQuery.getResultList()) {
  ...
  private void calculateAnzahlImpliziteAbmeldungenVorhergehendesSemesterSchueler(
      Semester previousSemesterBeforeSemesterbeginn, EntityManager entityManager) {
    TypedQuery<Object[]> typedQuery =
        entityManager.createQuery(
            "select k.schueler.personId, count(k) from Kursanmeldung k where"
                + " k.kurs.semester.semesterId = :semesterIdPreviousSemester and k.abmeldedatum is null group by k.schueler.personId",
            Object[].class);
    typedQuery.setParameter(
        "semesterIdPreviousSemester", previousSemesterBeforeSemesterbeginn.getSemesterId());
    for (Object[] result : typedQuery.getResultList()) {
      Integer schuelerId = (Integer) result[0];
  ...
  private void calculateAnzahlAnmeldungenAbmeldungenTotal() {
    for (Integer schuelerId : schuelerIdsAll) {
      if (schuelerIdsAnzahlAnmeldungen.containsKey(schuelerId)
          && schuelerIdsAnzahlAbmeldungen.containsKey(schuelerId)) {
        int netto =
            schuelerIdsAnzahlAnmeldungen.get(schuelerId)
                - schuelerIdsAnzahlAbmeldungen.get(schuelerId);
        if (netto > 0) {
  ...
  private void determineStatistikMonatEnde() {
    if (monatJahr.get(Calendar.MONTH) == Calendar.DECEMBER) {
      ...
      statistikMonatEnde =
          new GregorianCalendar(monatJahr.get(Calendar.YEAR), monatJahr.get(Calendar.MONTH) + 1, 1);
    }
  ...
  private Semester getRelevantesSemester(List<Semester> semestersAll) {
    FindSemesterForCalendarCommand findSemesterForCalendarCommand =
        new FindSemesterForCalendarCommand(statistikMonatEnde, semestersAll);
  ...
  private Semester checkIfMonatContainsSemesterbeginnAndGetPreviousSemester() {
    ...
    for (int i = 0; i < semestersAll.size(); i++) {
      if (!semestersAll.get(i).getSemesterbeginn().before(statistikMonatBeginn)
          && !semestersAll.get(i).getSemesterbeginn().after(statistikMonatEnde)) {
