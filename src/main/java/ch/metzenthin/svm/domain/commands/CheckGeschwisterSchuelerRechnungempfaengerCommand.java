...
public class CheckGeschwisterSchuelerRechnungempfaengerCommand implements Command {
  ...
  private final List<Schueler>
      andereSchuelerMitVaterMutterOderDrittpersonAlsRechnungsempfaengerList = new ArrayList<>();

  public CheckGeschwisterSchuelerRechnungempfaengerCommand(Schueler schueler) {
    this(schueler, null, null, null, false);
  }

  public CheckGeschwisterSchuelerRechnungempfaengerCommand(
      Schueler schueler, boolean isRechnungsempfaengerDrittperson) {
    ...
    if (vater != null) {
      for (Schueler kind : vater.getKinderVater()) {
        if (!kind.isIdenticalWith(schueler)) {
          ...
          if (kind.isAngemeldet()
              && !checkIfSchuelerAlreadyInList(kind, angemeldeteGeschwisterList)) {
            angemeldeteGeschwisterList.add(kind);
          }
        }
      }
  ...
  private void determineAndereSchuelerMitVaterMutterOderDrittpersonAlsRechnungsempfaenger() {
    ...
    if (mutter != null) {
      for (Schueler schueler1 : mutter.getSchuelerRechnungsempfaenger()) {
        ...
        if (!schueler1.isIdenticalWith(schueler)
            && schueler1.isAngemeldet()
            && !checkIfSchuelerAlreadyInList(schueler1, angemeldeteGeschwisterList)) {
    ...
    if (vater != null) {
      for (Schueler schueler1 : vater.getSchuelerRechnungsempfaenger()) {
        if (!schueler1.isIdenticalWith(schueler)
            && schueler1.isAngemeldet()
            && !checkIfSchuelerAlreadyInList(schueler1, angemeldeteGeschwisterList)
            && !checkIfSchuelerAlreadyInList(
                schueler1, andereSchuelerMitVaterMutterOderDrittpersonAlsRechnungsempfaengerList)) {
          andereSchuelerMitVaterMutterOderDrittpersonAlsRechnungsempfaengerList.add(schueler1);
        }
      }
    }

    // Rechnungsempfänger Drittperson (d.h. weder Mutter noch Vater ist identisch mit
    // Rechnungsempfänger)
