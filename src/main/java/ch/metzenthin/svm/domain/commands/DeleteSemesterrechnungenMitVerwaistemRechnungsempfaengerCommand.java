...
public class DeleteSemesterrechnungenMitVerwaistemRechnungsempfaengerCommand implements Command {
  ...
  public void execute() {
    ...
    while (it.hasNext()) {
      ...
      if ((rechnungsempfaenger.getSchuelerRechnungsempfaenger() == null
              || rechnungsempfaenger.getSchuelerRechnungsempfaenger().isEmpty())
          // Rechnungsempfänger von früheren (bezahlten) Rechnungen sind inzwischen möglicherweise
          // verwaist (falls Rechnungsempfänger gewechselt hatte)
        ...
        CheckIfAngehoerigerVerwaistAndDeleteCommand checkIfAngehoerigerVerwaistAndDeleteCommand =
            new CheckIfAngehoerigerVerwaistAndDeleteCommand(rechnungsempfaenger);
