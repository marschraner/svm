...
public class CreateSchuelerlisteCsvFileCommand extends CreateListeCommand {
  ...
  public CreateSchuelerlisteCsvFileCommand(
    ...
    try {
      ...
      for (Schueler schueler : schuelerSuchenTableModel.getSelektierteSchuelerList()) {
        ...
        if (schueler.getMutter() != null) {
          out.write(
              nachnameGratiskindFormatter.format(schueler.getMutter().getNachname())
                  + " "
                  + schueler.getMutter().getVorname());
        }
        out.write(separator);
        if (schueler.getMutter() != null
            && (schueler.getMutter().getWuenschtEmails() != null
                && schueler.getMutter().getWuenschtEmails())
            && schueler.getMutter().getEmail() != null) {
          out.write(schueler.getMutter().getEmail());
        }
        out.write(separator);
        if (schueler.getVater() != null) {
          out.write(
              nachnameGratiskindFormatter.format(schueler.getVater().getNachname())
                  + " "
                  + schueler.getVater().getVorname());
        }
        out.write(separator);
        if (schueler.getVater() != null
            && (schueler.getVater().getWuenschtEmails() != null
                && schueler.getVater().getWuenschtEmails())
            && schueler.getVater().getEmail() != null) {
          out.write(schueler.getVater().getEmail());
        }
        out.write(separator);
        String rechnungsempfaenger;
        if (schueler.getMutter() != null
            && schueler.getMutter().isIdenticalWith(schueler.getRechnungsempfaenger())) {
          rechnungsempfaenger = "Mutter";
        } else if (schueler.getVater() != null
            && schueler.getVater().isIdenticalWith(schueler.getRechnungsempfaenger())) {
          rechnungsempfaenger = "Vater";
        } else {
          rechnungsempfaenger =
              nachnameGratiskindFormatter.format(schueler.getRechnungsempfaenger().getNachname())
                  + " "
                  + schueler.getRechnungsempfaenger().getVorname();
        }
        ...
        Maercheneinteilung maercheneinteilung =
            schuelerSuchenTableModel.getMaercheneinteilungen().get(schueler);
  ...
  public String getAnmeldungen(Schueler schueler) {
    ...
    StringBuilder anmeldungenAsStr =
        new StringBuilder(anmeldungen.get(anmeldungen.size() - 1).toString());
