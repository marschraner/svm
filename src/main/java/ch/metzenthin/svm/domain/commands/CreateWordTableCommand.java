...
public class CreateWordTableCommand implements Command {
  ...
  private final List<List<int[]>> maxLengths;
  ...
  CreateWordTableCommand(
      List<List<String>> headerRows,
      List<List<List<String>>> datasets,
      List<Integer> columnWidths,
      List<List<Boolean>> boldCells,
      List<List<Integer>> mergedCells,
      List<List<int[]>> maxLengths,
      String title1,
      String title2,
      File outputFile,
      int topMargin,
      ...
      int leftMargin,
      int rightMargin,
      int numberOfDatasetsFirstPage,
      int numberOfDatasetsNormalPage) {
  ...
  public void execute() {
    ...
    SetWordPageMarginsCommand setWordPageMarginsCommand =
        new SetWordPageMarginsCommand(
            wordMLPackage, objectFactory, topMargin, bottomMargin, leftMargin, rightMargin);
  ...
  private void addTable(int datasetNumberStart, int numberOfDatasetsPerPage, boolean header) {
    ...
    for (int n = datasetNumberStart; n < datasetNumberStart + numberOfDatasetsPerPage; n++) {
      ...
      for (int i = 0; i < datasetRows.size(); i++) {
        ...
        List<Integer> mergedRow = mergedCells.get(i);
        List<int[]> maxLengthsRow = maxLengths.get(i);
        ...
        while (j < datasetRow.size()) {
          ...
          while (k < maxLengthsCell.length
              ...
              && maxLengthsCell[k] > 0
              && datasetRow.get(j).length() > maxLengthsCell[k]) {
