...
public class UpdateWochenbetragUndAnzWochenCommand implements Command {
  ...
  public UpdateWochenbetragUndAnzWochenCommand(
      Angehoeriger rechnungsempfaenger, Semester currentSemester) {
  ...
  public void execute() {
    ...
    for (Semesterrechnung semesterrechnung1 : rechnungsempfaenger.getSemesterrechnungen()) {
      ...
      if (nextSemester != null
          && semesterrechnung1.getSemester().getSemesterId().equals(nextSemester.getSemesterId())) {
    ...
    if (semesterrechnungCurrentSemester != null) {
      ...
      CalculateAnzWochenCommand calculateAnzWochenCommand =
          new CalculateAnzWochenCommand(
              semesterrechnungCurrentSemester
                  ...
                  .getSchuelerRechnungsempfaenger(),
              currentSemester);
      ...
      if (semesterrechnungCurrentSemester.getRechnungsdatumVorrechnung() == null) {
        semesterrechnungCurrentSemester.setAnzahlWochenVorrechnung(
            calculateAnzWochenCommand.getAnzahlWochen());
        dataChanged = true;
      }
      ...
      if (semesterrechnungCurrentSemester.getRechnungsdatumNachrechnung() == null) {
        semesterrechnungCurrentSemester.setAnzahlWochenNachrechnung(
            calculateAnzWochenCommand.getAnzahlWochen());
        ...
        if (calculateAnzWochenCommand.getResult()
            != CalculateAnzWochenCommand.Result.ALLE_KURSE_GLEICHE_ANZAHL_WOCHEN) {
          result = calculateAnzWochenCommand.getResult();
        }
        if (calculateWochenbetragCommand.getResult()
            == CalculateWochenbetragCommand.Result.WOCHENBETRAG_ERFOLGREICH_BERECHNET) {
          semesterrechnungCurrentSemester.setWochenbetragNachrechnung(
              calculateWochenbetragCommand.getWochenbetrag());
    ...
    if (semesterrechnungNextSemester != null
        && semesterrechnungNextSemester.getRechnungsdatumVorrechnung() == null) {
      CalculateWochenbetragCommand calculateWochenbetragCommand =
          new CalculateWochenbetragCommand(
      ...
      if (calculateWochenbetragCommand.getResult()
          == CalculateWochenbetragCommand.Result.WOCHENBETRAG_ERFOLGREICH_BERECHNET) {
        semesterrechnungNextSemester.setWochenbetragVorrechnung(
            calculateWochenbetragCommand.getWochenbetrag());
      } else { // sollte nie eintreten
