...
public class SchuelerSuchenCommand implements Command {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(SchuelerSuchenCommand.class);
  ...
  public SchuelerSuchenCommand(SchuelerSuchenModel schuelerSuchenModel) {
    ...
    this.geburtsdatumSuchperiodeDateFormatString =
        schuelerSuchenModel.getGeburtsdatumSuchperiodeDateFormatString();
    ...
    this.maerchenFuerSucheBeruecksichtigen =
  ...
  public void execute() {
    ...
    // Sortierung in Java (weil SELECT DISTINCT ... ORDER BY ... s.adresse.ort, s.adresse.strasse
    // nicht mehr erlaubt ab MySQL 5.7,
    // vgl.
    // http://www.programmerinterview.com/index.php/database-sql/sql-select-distinct-and-order-by/)
  ...
  private void createWhereSelectionsStammdatenMutter() {
    // Einfache or-Abfrage für Eltern / alle (where s.vorname = :vorname or s.mutter.vorname =
    // :vorname or s.vater.vorname = :vorname)
    // schlägt fehl, wenn mutter oder vater nicht existiert, auch wenn ein anderes or-Element, z.B.
    // s.vorname = :vorname erfüllt wäre. Bug?
    // Abhilfe: Subselects.
    if (person != null && checkNotEmpty(person.getVorname())) {
      selectStatementSb.append(
          " exists (select s1 from Schueler s1 where lower(s1.mutter.vorname) like :vorname and s1.personId = s.personId) and");
    }
    if (person != null && checkNotEmpty(person.getNachname())) {
      selectStatementSb.append(
          " exists (select s1 from Schueler s1 where lower(s1.mutter.nachname) like :nachname and s1.personId = s.personId) and");
    }
    if (adresse != null && checkNotEmpty(adresse.getStrasse())) {
      selectStatementSb.append(
          " (lower(s.adresse.strasse) like :strasse or exists (select s1 from Schueler s1 where lower(s1.mutter.adresse.strasse) like :strasse and s1.personId = s.personId)) and");
    }
    if (adresse != null && checkNotEmpty(adresse.getHausnummer())) {
      selectStatementSb.append(
          " (lower(s.adresse.hausnummer) = :hausnummer or exists (select s1 from Schueler s1 where lower(s1.mutter.adresse.hausnummer) = :hausnummer and s1.personId = s.personId)) and");
    }
    if (adresse != null && checkNotEmpty(adresse.getPlz())) {
      selectStatementSb.append(
          " (lower(s.adresse.plz) like :plz or exists (select s1 from Schueler s1 where lower(s1.mutter.adresse.plz) like :plz and s1.personId = s.personId)) and");
    }
    if (adresse != null && checkNotEmpty(adresse.getOrt())) {
      selectStatementSb.append(
          " (lower(s.adresse.ort) like :ort or exists (select s1 from Schueler s1 where lower(s1.mutter.adresse.ort) like :ort and s1.personId = s.personId)) and");
    }
    if (person != null && checkNotEmpty(person.getFestnetz())) {
      selectStatementSb.append(
          " (replace(s.festnetz, ' ', '') = :festnetz or exists (select s1 from Schueler s1 where replace(s1.mutter.festnetz, ' ', '') = :festnetz and s1.personId = s.personId)) and");
    }
    if (person != null && checkNotEmpty(person.getNatel())) {
      selectStatementSb.append(
          " (replace(s.natel, ' ', '') = :natel or exists (select s1 from Schueler s1 where replace(s1.mutter.natel, ' ', '') = :natel and s1.personId = s.personId)) and");
    }
    if (person != null && checkNotEmpty(person.getEmail())) {
      selectStatementSb.append(
          " (lower(s.email) like :email or exists (select s1 from Schueler s1 where lower(s1.mutter.email) like :email and s1.personId = s.personId)) and");
    }
    selectStatementSb.setLength(selectStatementSb.length() - 4); // letztes ' and' löschen
  }
  ...
  private void createWhereSelectionsStammdatenVater() {
    if (person != null && checkNotEmpty(person.getVorname())) {
      selectStatementSb.append(
          " exists (select s1 from Schueler s1 where lower(s1.vater.vorname) like :vorname and s1.personId = s.personId) and");
    }
    if (person != null && checkNotEmpty(person.getNachname())) {
      selectStatementSb.append(
          " exists (select s1 from Schueler s1 where lower(s1.vater.nachname) like :nachname and s1.personId = s.personId) and");
    }
    if (adresse != null && checkNotEmpty(adresse.getStrasse())) {
      selectStatementSb.append(
          " (lower(s.adresse.strasse) like :strasse or exists (select s1 from Schueler s1 where lower(s1.vater.adresse.strasse) like :strasse and s1.personId = s.personId)) and");
    }
    if (adresse != null && checkNotEmpty(adresse.getHausnummer())) {
      selectStatementSb.append(
          " (lower(s.adresse.hausnummer) = :hausnummer or exists (select s1 from Schueler s1 where lower(s1.vater.adresse.hausnummer) = :hausnummer and s1.personId = s.personId)) and");
    }
    if (adresse != null && checkNotEmpty(adresse.getPlz())) {
      selectStatementSb.append(
          " (lower(s.adresse.plz) like :plz or exists (select s1 from Schueler s1 where lower(s1.vater.adresse.plz) like :plz and s1.personId = s.personId)) and");
    }
    if (adresse != null && checkNotEmpty(adresse.getOrt())) {
      selectStatementSb.append(
          " (lower(s.adresse.ort) like :ort or exists (select s1 from Schueler s1 where lower(s1.vater.adresse.ort) like :ort and s1.personId = s.personId)) and");
    }
    if (person != null && checkNotEmpty(person.getFestnetz())) {
      selectStatementSb.append(
          " (replace(s.festnetz, ' ', '') = :festnetz or exists (select s1 from Schueler s1 where replace(s1.vater.festnetz, ' ', '') = :festnetz and s1.personId = s.personId)) and");
    }
    if (person != null && checkNotEmpty(person.getNatel())) {
      selectStatementSb.append(
          " (replace(s.natel, ' ', '') = :natel or exists (select s1 from Schueler s1 where replace(s1.vater.natel, ' ', '') = :natel and s1.personId = s.personId)) and");
    }
    if (person != null && checkNotEmpty(person.getEmail())) {
      selectStatementSb.append(
          " (lower(s.email) like :email or exists (select s1 from Schueler s1 where lower(s1.vater.email) like :email and s1.personId = s.personId)) and");
    }
    selectStatementSb.setLength(selectStatementSb.length() - 4); // letztes ' and' löschen
  }
  ...
  private void createWhereSelectionsStammdatenRechnungsempfaenger() {
    ...
    if (adresse != null && checkNotEmpty(adresse.getHausnummer())) {
      selectStatementSb.append(
          " lower(s.rechnungsempfaenger.adresse.hausnummer) = :hausnummer and");
    }
    if (adresse != null && checkNotEmpty(adresse.getPlz())) {
  ...
  private void createWhereSelectionsGeburtsdatumSuchperiode() {
    ...
    if (geburtsdatumSuchperiodeDateFormatString.equals("dd.MM.")) {
      selectStatementSb.append(
          " function('DAY', s.geburtsdatum) = function('DAY', :geburtsdatumSuchperiodeBeginn) and function('MONTH', s.geburtsdatum) = function('MONTH', :geburtsdatumSuchperiodeBeginn) and");
    }
    if (geburtsdatumSuchperiodeDateFormatString.equals("MM.yyyy")) {
      if (geburtsdatumSuchperiodeBeginn != null && geburtsdatumSuchperiodeEnde != null) {
        selectStatementSb.append(
            " s.geburtsdatum >= :geburtsdatumSuchperiodeBegMmYyyy and s.geburtsdatum < :geburtsdatumSuchperiodeEndMmYyyy and");
      } else if (geburtsdatumSuchperiodeBeginn != null) {
        selectStatementSb.append(
            " function('MONTH', s.geburtsdatum) = function('MONTH', :geburtsdatumSuchperiodeBeginn) and function('YEAR', s.geburtsdatum) = function('YEAR', :geburtsdatumSuchperiodeBeginn) and");
      }
    }
    if (geburtsdatumSuchperiodeDateFormatString.equals("dd.MM.yyyy")) {
      if (geburtsdatumSuchperiodeBeginn != null && geburtsdatumSuchperiodeEnde != null) {
        selectStatementSb.append(
            " s.geburtsdatum >= :geburtsdatumSuchperiodeBeginn and s.geburtsdatum <= :geburtsdatumSuchperiodeEnde and");
      } else if (geburtsdatumSuchperiodeBeginn != null) {
    ...
    if (geburtsdatumSuchperiodeDateFormatString.equals("yyyy")) {
      if (geburtsdatumSuchperiodeBeginn != null && geburtsdatumSuchperiodeEnde != null) {
        selectStatementSb.append(
            " function('YEAR', s.geburtsdatum) >= function('YEAR', :geburtsdatumSuchperiodeBeginn) and function('YEAR', s.geburtsdatum) <= function('YEAR', :geburtsdatumSuchperiodeEnde) and");
      } else if (geburtsdatumSuchperiodeBeginn != null) {
        selectStatementSb.append(
            " function('YEAR', s.geburtsdatum) = function('YEAR', :geburtsdatumSuchperiodeBeginn) and");
      }
    }
  }
  ...
  private void createWhereSelectionsDispensation() {
    ...
    if (dispensation == SchuelerSuchenModel.DispensationSelected.DISPENSIERT) {
      selectStatementSb.append(
          " exists (select dis from Dispensation dis join dis.schueler sch where dis.dispensationsbeginn <= :stichtag and (dis.dispensationsende is null or dis.dispensationsende >= :stichtag) and s.personId = sch.personId) and");
    }
    if (dispensation == SchuelerSuchenModel.DispensationSelected.NICHT_DISPENSIERT) {
      selectStatementSb.append(
          " not exists (select dis from Dispensation dis join dis.schueler sch where dis.dispensationsbeginn <= :stichtag and (dis.dispensationsende is null or dis.dispensationsende >= :stichtag) and s.personId = sch.personId) and");
    }
  }
  ...
  private void createWhereSelectionsAnmeldestatus() {
    ...
    if (anmeldestatus == SchuelerSuchenModel.AnmeldestatusSelected.ANGEMELDET) {
      selectStatementSb.append(
          " exists (select anm from Anmeldung anm join anm.schueler sch where anm.anmeldedatum <= :stichtag and (anm.abmeldedatum is null or anm.abmeldedatum >= :stichtag) and s.personId = sch.personId) and");
    }
    if (anmeldestatus == SchuelerSuchenModel.AnmeldestatusSelected.ABGEMELDET) {
      selectStatementSb.append(
          " not exists (select anm from Anmeldung anm join anm.schueler sch where anm.anmeldedatum <= :stichtag and (anm.abmeldedatum is null or anm.abmeldedatum >= :stichtag) and s.personId = sch.personId) and");
    }
  }
    ...
    if (maerchenFuerSucheBeruecksichtigen) {
      ...
      if (checkNotEmpty(rollen)) {
        ...
        for (int i = 0; i < rollen.split("[,;]").length; i++) {
          selectStatementSb
              .append(" lower(trim(mae.rolle1)) = :maerchenrolleEq")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle1)) like :maerchenrolleL1")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle1)) like :maerchenrolleL2")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle1)) like :maerchenrolleL3")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle1)) like :maerchenrolleL4")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle1)) like :maerchenrolleL5")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle2)) = :maerchenrolleEq")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle2)) like :maerchenrolleL1")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle2)) like :maerchenrolleL2")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle2)) like :maerchenrolleL3")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle2)) like :maerchenrolleL4")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle2)) like :maerchenrolleL5")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle3)) = :maerchenrolleEq")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle3)) like :maerchenrolleL1")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle3)) like :maerchenrolleL2")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle3)) like :maerchenrolleL3")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle3)) like :maerchenrolleL4")
              .append(i)
              .append(" or");
          selectStatementSb
              .append(" lower(trim(mae.rolle3)) like :maerchenrolleL5")
              .append(i)
              .append(" or");
        }
        // letztes or löschen
  ...
  private void setParameterGeburtsdatumSuchperiode() {
    ...
    if (selectStatementSb.toString().contains(":geburtsdatumSuchperiodeBegMmYyyy")) {
      Calendar geburtsdatumSuchperiodeBeginnMmYyyy =
          new GregorianCalendar(
              geburtsdatumSuchperiodeBeginn.get(Calendar.YEAR),
              geburtsdatumSuchperiodeBeginn.get(Calendar.MONTH),
              1);
      typedQuery.setParameter(
          "geburtsdatumSuchperiodeBegMmYyyy", geburtsdatumSuchperiodeBeginnMmYyyy);
    }
    if (selectStatementSb.toString().contains(":geburtsdatumSuchperiodeEndMmYyyy")) {
      ...
      if (geburtsdatumSuchperiodeEnde.get(Calendar.MONTH) == Calendar.DECEMBER) {
        geburtsdatumSuchperiodeEndeMmYyyy =
            new GregorianCalendar(
                geburtsdatumSuchperiodeEnde.get(Calendar.YEAR) + 1, Calendar.JANUARY, 1);
      } else {
        //noinspection MagicConstant
        geburtsdatumSuchperiodeEndeMmYyyy =
            new GregorianCalendar(
                geburtsdatumSuchperiodeEnde.get(Calendar.YEAR),
                geburtsdatumSuchperiodeEnde.get(Calendar.MONTH) + 1,
                1);
      }
      typedQuery.setParameter(
          "geburtsdatumSuchperiodeEndMmYyyy", geburtsdatumSuchperiodeEndeMmYyyy);
    }
  }
