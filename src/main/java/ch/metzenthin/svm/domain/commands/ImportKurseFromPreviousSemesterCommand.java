...
public class ImportKurseFromPreviousSemesterCommand implements Command {
  ...
  public ImportKurseFromPreviousSemesterCommand(
      List<Kurs> kurseCurrentSemester, Semester currentSemester) {
  ...
  public void execute() {
    ...
    if (currentSemester.getSemesterbezeichnung() == Semesterbezeichnung.ERSTES_SEMESTER) {
      FindSemesterOneYearBeforeCommand findSemesterOneYearBeforeCommand =
          new FindSemesterOneYearBeforeCommand(currentSemester);
    ...
    for (Kurs kursPreviousSemester : kursePreviousSemester) {
      ...
      if (currentSemester.getSemesterbezeichnung() == Semesterbezeichnung.ZWEITES_SEMESTER) {
        for (Kursanmeldung kursanmeldungPreviousSemester :
            kursPreviousSemester.getKursanmeldungen()) {
          ...
          if (kursanmeldungPreviousSemester.getAbmeldedatum() == null
              && (anmeldung.getAbmeldedatum() == null
                  || anmeldung.getAbmeldedatum().after(currentSemester.getSemesterbeginn()))) {
            Kursanmeldung kursanmeldung =
                new Kursanmeldung(
                    kursanmeldungPreviousSemester.getSchueler(),
                    kurs,
                    currentSemester.getSemesterbeginn(),
                    null,
                    null);
            ...
            UpdateWochenbetragUndAnzWochenCommand updateWochenbetragUndAnzWochenCommand =
                new UpdateWochenbetragUndAnzWochenCommand(
                    kursanmeldungPreviousSemester.getSchueler().getRechnungsempfaenger(),
                    currentSemester);
