 ...
 */
public class CreateSemesterrechnungenRechnungsempfaengerWithPreviousSettingsCommand
    implements Command {
  ...
  public CreateSemesterrechnungenRechnungsempfaengerWithPreviousSettingsCommand(
      List<Angehoeriger> rechnungsempfaengers, Semester currentSemester) {
    ...
    for (Angehoeriger rechnungsempfaenger : rechnungsempfaengers) {
      ...
      CalculateAnzWochenCommand calculateAnzWochenCommand =
          new CalculateAnzWochenCommand(
              rechnungsempfaenger.getSchuelerRechnungsempfaenger(), currentSemester);
      ...
      if (previousSemester != null) {
        calculateWochenbetragCommand =
            new CalculateWochenbetragCommand(
                semesterrechnung, previousSemester, Rechnungstyp.VORRECHNUNG, lektionsgebuehrenMap);
        ...
        if (calculateWochenbetragCommand.getResult()
          ...
          semesterrechnung.setWochenbetragVorrechnung(
      ...
      calculateWochenbetragCommand =
          new CalculateWochenbetragCommand(
              semesterrechnung, currentSemester, Rechnungstyp.NACHRECHNUNG, lektionsgebuehrenMap);
      calculateWochenbetragCommand.execute();
      if (calculateWochenbetragCommand.getResult()
          == CalculateWochenbetragCommand.Result.WOCHENBETRAG_ERFOLGREICH_BERECHNET) {
        semesterrechnung.setWochenbetragNachrechnung(
            calculateWochenbetragCommand.getWochenbetrag());
      } else { // sollte nie eintreten
        semesterrechnung.setWochenbetragVorrechnung(new BigDecimal("-99999.99"));
      }
