...
public class MonatsstatistikSchuelerSuchenCommand implements Command {
  ...
  // input
  private final MonatsstatistikSchuelerModel.AnAbmeldungenDispensationenSelected
      anAbmeldungenDispensationen;
  ...
  public MonatsstatistikSchuelerSuchenCommand(
      MonatsstatistikSchuelerModel monatsstatistikSchuelerModel) {
    this.anAbmeldungenDispensationen =
        monatsstatistikSchuelerModel.getAnAbmeldungenDispensationen();
    ...
    switch (anAbmeldungenDispensationen) {
      case ANMELDUNGEN_KINDERTHEATER ->
          selectStatementSb.append(
              "select s from Schueler s where exists (select anm from Anmeldung anm join anm.schueler sch where anm.anmeldedatum >= :statistikMonatBeginn and anm.anmeldedatum <= :statistikMonatEnde and s.personId = sch.personId) and");
      case ABMELDUNGEN_KINDERTHEATER ->
          selectStatementSb.append(
              "select s from Schueler s where exists (select anm from Anmeldung anm join anm.schueler sch where anm.abmeldedatum >= :statistikMonatBeginn and anm.abmeldedatum <= :statistikMonatEnde and s.personId = sch.personId) and");
      case ANMELDUNGEN_KURSE ->
          selectStatementSb.append(
              "select distinct s from Schueler s join s.kursanmeldungen k where k.anmeldedatum >= :statistikMonatBeginn and k.anmeldedatum <= :statistikMonatEnde and");
      case ABMELDUNGEN_KURSE ->
          selectStatementSb.append(
              "select distinct s from Schueler s join s.kursanmeldungen k where k.abmeldedatum >= :statistikMonatBeginn and k.abmeldedatum <= :statistikMonatEnde and");
      case DISPENSATIONEN ->
          selectStatementSb.append(
              "select s from Schueler s where exists (select dis from Dispensation dis join dis.schueler sch where dis.dispensationsbeginn <= :statistikMonatEnde and (dis.dispensationsende is null or dis.dispensationsende >= :statistikMonatBeginn) and s.personId = sch.personId) and");
    }
  }
  ...
  private void setParameterStatistikMonat() {
    if (selectStatementSb.toString().contains(":statistikMonatBeginn")) {
      Calendar statistikMonatBeginn =
          new GregorianCalendar(monatJahr.get(Calendar.YEAR), monatJahr.get(Calendar.MONTH), 1);
    ...
    if (selectStatementSb.toString().contains(":statistikMonatEnde")) {
      ...
      if (monatJahr.get(Calendar.MONTH) == Calendar.DECEMBER) {
        statistikMonatEnde =
            new GregorianCalendar(monatJahr.get(Calendar.YEAR) + 1, Calendar.JANUARY, 1);
      } else {
        //noinspection MagicConstant
        statistikMonatEnde =
            new GregorianCalendar(
                monatJahr.get(Calendar.YEAR), monatJahr.get(Calendar.MONTH) + 1, 1);
      }
