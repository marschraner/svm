...
public class SemesterrechnungenModelImpl extends AbstractModel implements SemesterrechnungenModel {
  ...
  @Override
  public SemesterrechnungBearbeitenModel getSemesterrechnungBearbeitenModel(
    ...
    SemesterrechnungBearbeitenModel semesterrechnungBearbeitenModel =
        svmContext.getModelFactory().createSemesterrechnungBearbeitenModel();
    Semesterrechnung semesterrechnungSelected =
        semesterrechnungenTableModel.getSemesterrechnungSelected(rowSelected);
    semesterrechnungBearbeitenModel.setSemesterrechnungOrigin(semesterrechnungSelected);
    return semesterrechnungBearbeitenModel;
  }

  @Override
  public void semesterrechnungPhysischLoeschen(
    ...
    DeleteSemesterrechnungCommand deleteSemesterrechnungCommand =
        new DeleteSemesterrechnungCommand(
            semesterrechnungenTableModel.getSemesterrechnungen(), rowSelected);
    commandInvoker.executeCommandAsTransaction(deleteSemesterrechnungCommand);
  }

  @Override
  public void semesterrechnungLogischLoeschen(
      SemesterrechnungenTableModel semesterrechnungenTableModel, int rowSelected) {
    CommandInvoker commandInvoker = getCommandInvoker();
    DeleteSemesterrechnungLogicallyCommand deleteSemesterrechnungCommand =
        new DeleteSemesterrechnungLogicallyCommand(
            semesterrechnungenTableModel.getSemesterrechnungen(), rowSelected);
    commandInvoker.executeCommandAsTransaction(deleteSemesterrechnungCommand);
  }

  @Override
  public void semesterrechnungWiederherstellen(
      SemesterrechnungenTableModel semesterrechnungenTableModel, int rowSelected) {
    CommandInvoker commandInvoker = getCommandInvoker();
    RestoreSemesterrechnungCommand restoreSemesterrechnungCommand =
        new RestoreSemesterrechnungCommand(
            semesterrechnungenTableModel.getSemesterrechnungen(), rowSelected);
  ...
  @Override
  public boolean hasSemesterrechnungKurse(
      SemesterrechnungenTableModel semesterrechnungenTableModel, int rowSelected) {
    Semesterrechnung semesterrechnung =
        semesterrechnungenTableModel.getSemesterrechnungen().get(rowSelected);
    ...
    if (previousSemester != null) {
      for (Schueler schueler : schuelersRechnungsempfaenger) {
        for (Kursanmeldung kursanmeldung : schueler.getSortedKursanmeldungen()) {
          Kurs kurs = kursanmeldung.getKurs();
          if (!kurs.getSemester().getSemesterId().equals(previousSemester.getSemesterId())
