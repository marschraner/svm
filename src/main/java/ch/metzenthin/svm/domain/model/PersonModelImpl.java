...
abstract class PersonModelImpl extends AbstractModel implements PersonModel {
  ...
  private final StringModelAttribute natelModelAttribute =
      new StringModelAttribute(
          this,
          Field.NATEL,
          13,
  ...
  final StringModelAttribute emailModelAttribute =
      new StringModelAttribute(
          this,
          Field.EMAIL,
          1,
  ...
  private final CalendarModelAttribute geburtsdatumModelAttribute =
      new CalendarModelAttribute(
          this,
          Field.GEBURTSDATUM,
          getEarliestValidDateGeburtstag(),
          getLatestValidDateGeburtstag(),
          new AttributeAccessor<>() {
            @Override
  ...
  private final StringModelAttribute strasseHausnummerModelAttribute =
      new StringModelAttribute(
          this,
  ...
  private final StringModelAttribute plzModelAttribute =
      new StringModelAttribute(
          this,
          Field.PLZ,
          4,
  ...
  private final StringModelAttribute ortModelAttribute =
      new StringModelAttribute(
          this,
          Field.ORT,
          1,
  ...
  private final StringModelAttribute festnetzModelAttribute =
      new StringModelAttribute(
          this,
          Field.FESTNETZ,
          13,
  ...
  public boolean isCompleted() {
    ...
    return (!isSetAnyNameElement() || isSetName())
        && (!isSetAnyAdresseElement() || (isSetName() && isSetAdresse()));
  }
  ...
  public void doValidate() throws SvmValidationException {
    if (isAdresseRequired()) {
      if (!isSetName()) {
        throw new SvmValidationException(
            2000, "Nachname und Vorname obligatorisch", Field.NACHNAME);
      }
      if (!isSetAdresse()) {
      ...
      if (isSetAnyNameElement() && !isSetName()) {
        throw new SvmValidationException(
            2002, "Nachname und Vorname müssen zusammen angegeben werden", Field.NACHNAME);
      }
      if (isSetAnyAdresseElement()) {
        if (!isSetName()) {
          throw new SvmValidationException(
              2003,
              "Nachname und Vorname müssen angegeben werden, wenn eine Adresse vorhanden ist",
              Field.NACHNAME);
        }
        if (!isSetAdresse()) {
    ...
    if (checkNotEmpty(getPerson().getFestnetz()) && !isSetName()) {
      throw new SvmValidationException(
          2005,
          "Nachname und Vorname müssen angegeben werden, wenn ein Festnetz erfasst ist",
          Field.NACHNAME);
    }
    if (checkNotEmpty(getPerson().getNatel()) && !isSetName()) {
      throw new SvmValidationException(
          2006,
          "Nachname und Vorname müssen angegeben werden, wenn ein Natel erfasst ist",
          Field.NACHNAME);
    }
    if (checkNotEmpty(getPerson().getEmail()) && !isSetName()) {
      throw new SvmValidationException(
          2007,
          "Nachname und Vorname müssen angegeben werden, wenn eine E-Mail erfasst ist",
          Field.NACHNAME);
    }
  }
  ...
  private boolean isSetAnyNameElement() {
    return checkNotEmpty(getPerson().getNachname()) || checkNotEmpty(getPerson().getVorname());
  }
  ...
  private boolean isSetAdresse() {
    return checkNotEmpty(adresse.getPlz()) && checkNotEmpty(adresse.getOrt());
  }
