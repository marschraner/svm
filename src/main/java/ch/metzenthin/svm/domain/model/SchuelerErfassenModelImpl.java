...
public class SchuelerErfassenModelImpl extends AbstractModel
  ...
  private boolean isSubModelsCompleted() {
    return isSchuelerModelCompleted
        && isMutterModelCompleted
        && isVaterModelCompleted
        && isDrittempfaengerModelCompleted;
  }

  /**
   * Überschrieben, damit Observer dieses Modells nicht informiert wird. Er muss die Exception
   * fangen und entsprechend handeln.
   */
  @Override
    ...
    if (isSubModelsCompleted()) {
      if (!isSetRechnungsempfaenger()) {
        throw new SvmValidationException(
            3000, "Rechnungsempfänger ist nicht gesetzt", Field.RECHNUNGSEMPFAENGER);
      }
  ...
  private boolean isSetRechnungsempfaenger() {
    return mutterModel.isRechnungsempfaenger()
        || vaterModel.isRechnungsempfaenger()
  ...
  private void replaceByStrasseHausnummerSchueler(AngehoerigerModel angehoerigerModel) {
    try {
      ...
      LOGGER.trace(
          "SchuelerErfassenController replaceByStrasseHausnummerSchueler RequiredException={}",
          e.getMessage());
  ...
  private void replaceByPlzSchueler(AngehoerigerModel angehoerigerModel) {
    try {
      ...
      LOGGER.trace(
          "SchuelerErfassenController replaceByPlzSchueler RequiredException={}", e.getMessage());
      // Keine weitere Aktion. Die Required-Prüfung erfolgt erneut, nachdem alle Field-Prüfungen
      // bestanden sind.
    } catch (SvmValidationException e) {
      // Tritt nie ein, da Validierung bereits beim Schüler
  ...
  private void replaceByOrtSchueler(AngehoerigerModel angehoerigerModel) {
    try {
      ...
      LOGGER.trace(
          "SchuelerErfassenController replaceByOrtSchueler RequiredException={}", e.getMessage());
      // Keine weitere Aktion. Die Required-Prüfung erfolgt erneut, nachdem alle Field-Prüfungen
      // bestanden sind.
    } catch (SvmValidationException e) {
      // Tritt nie ein, da Validierung bereits beim Schüler
  ...
  private void replaceByFestnetzSchueler(AngehoerigerModel angehoerigerModel) {
    try {
      ...
      LOGGER.trace(
          "SchuelerErfassenController replaceByFestnetzSchueler RequiredException={}",
          e.getMessage());
      // Keine weitere Aktion. Die Required-Prüfung erfolgt erneut, nachdem alle Field-Prüfungen
      // bestanden sind.
    } catch (SvmValidationException e) {
  ...
  @Override
  public SchuelerErfassenSaveResult proceedUebernehmen(
      SchuelerErfassenSaveResult schuelerErfassenSaveResult) {
    // Eigentlich müsste man das Model durch das neue Objekt, das übernommen wird, ersetzen. Darauf
    // verzichten
    // wir im Moment.
  ...
  @Override
  public SchuelerErfassenSaveResult proceedWeiterfahren(
      SchuelerErfassenSaveResult schuelerErfassenSaveResult) {
  ...
  @Override
  public SchuelerErfassenSaveResult speichern(
      SchuelerErfassenSaveResult schuelerErfassenSaveResult) {
    ...
    try {
      ...
      return new SchuelerErfassenUnerwarteterFehlerResult(
          ValidateSchuelerCommand.Result.UNERWARTETER_FEHLER, e);
    } finally {
  ...
  public void fruehereAnmeldungenLoeschen() {
    RemoveFruehereAnmeldungenFromSchuelerCommand removeFruehereAnmeldungenFromSchuelerCommand =
        new RemoveFruehereAnmeldungenFromSchuelerCommand(getSchuelerOrigin());
  ...
  public SchuelerSuchenTableData getSchuelerSuchenTableData() {
    return new SchuelerSuchenTableData(
        Collections.singletonList(getSchueler()),
        new HashMap<>(),
        null,
        null,
        null,
