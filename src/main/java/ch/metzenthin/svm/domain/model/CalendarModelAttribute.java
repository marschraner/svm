...
public class CalendarModelAttribute {
  ...
  CalendarModelAttribute(
      ...
      Calendar latestValidDate,
      AttributeAccessor<Calendar> attributeAccessor) {
  ...
  String getValueAsString(String dateFormatString) {
    ...
    return nullAsEmptyString(
        asString(getValue(), (dateFormatString != null) ? dateFormatString : ""));
  }
  ...
  void setNewValue(
    ...
    if (checkNotEmpty(newValueTrimmed)) {
      ...
      if (!isBulkUpdate && dateFormatString.contains("y") && newValueAsCalendar != null) {
  ...
  private void checkEarliestValidDate(Calendar newValueAsCalendar, String dateFormatString)
      throws SvmValidationException {
    if (newValueAsCalendar.before(earliestValidDate)) {
      modelAttributeListener.invalidate();
      throw new SvmValidationException(
          1200,
          "Datum darf nicht vor " + asString(earliestValidDate, dateFormatString) + " liegen",
          field);
    }
  }

  private void checkLatestValidDate(Calendar newValueAsCalendar, String dateFormatString)
      throws SvmValidationException {
    if (newValueAsCalendar.after(latestValidDate)) {
      modelAttributeListener.invalidate();
      throw new SvmValidationException(
          1200,
          "Datum darf nicht nach " + asString(latestValidDate, dateFormatString) + " liegen",
          field);
    }
  }
