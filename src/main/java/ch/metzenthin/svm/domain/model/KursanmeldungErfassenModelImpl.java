...
public class KursanmeldungErfassenModelImpl extends AbstractModel
  ...
  public void setAnmeldedatum(String anmeldedatum) throws SvmValidationException {
    ...
    if (!isBulkUpdate()
        ...
        && kursanmeldung.getAnmeldedatum().after(kursanmeldung.getAbmeldedatum())) {
    ...
    if (!isBulkUpdate()
        && kursanmeldung.getAnmeldedatum() != null
        && kursanmeldung.getAnmeldedatum().before(semester.getSemesterbeginn())) {
      kursanmeldung.setAnmeldedatum(null);
      invalidate();
      throw new SvmValidationException(
          2026,
          "Datum darf nicht vor " + asString(semester.getSemesterbeginn()) + " liegen",
          Field.ANMELDEDATUM);
    }
    if (!isBulkUpdate()
        && kursanmeldung.getAnmeldedatum() != null
        && kursanmeldung.getAnmeldedatum().after(semester.getSemesterende())) {
      kursanmeldung.setAnmeldedatum(null);
      invalidate();
      throw new SvmValidationException(
          2027,
          "Datum darf nicht nach " + asString(semester.getSemesterende()) + " liegen",
          Field.ANMELDEDATUM);
    }
  }
  ...
  public void setAbmeldedatum(String abmeldedatum) throws SvmValidationException {
    ...
    Calendar spaetestesGueltigesAbmeldedatum =
        (Calendar) getSemesterbeginnNaechstesSemester().clone();
    spaetestesGueltigesAbmeldedatum.add(Calendar.DAY_OF_YEAR, -1);
    if (!isBulkUpdate()
        && kursanmeldung.getAbmeldedatum() != null
        && kursanmeldung.getAbmeldedatum().after(spaetestesGueltigesAbmeldedatum)) {
      kursanmeldung.setAnmeldedatum(null);
      invalidate();
      throw new SvmValidationException(
          2028,
          "Datum darf nicht nach " + asString(spaetestesGueltigesAbmeldedatum) + " liegen",
          Field.ABMELDEDATUM);
    }
  }
  ...
  private Calendar getSemesterbeginnNaechstesSemester() {
    ...
    if (nextSemester != null) {
      semesterbeginnNaechstesSemester =
          findNextSemesterCommand.getNextSemester().getSemesterbeginn();
    }
  ...
  public Semester getInitSemester(List<Semester> semesterList) {
    DetermineSemesterInitCommand determineSemesterInitCommand =
        new DetermineSemesterInitCommand(semesterList, 40);
  ...
  public Calendar getInitAnmeldedatum(KursanmeldungenTableModel kursanmeldungenTableModel) {
    ...
    // Falls Anmeldedatum bereits gesetzt wurde, diese bei Semesterwechsel in jedem Fall
    // aktualisieren
    if (kursanmeldung.getAnmeldedatum() != null) {
      return kursanmeldungInit;
    }

    // Prüfen, ob Schüler Kursanmeldungen im vorhergehenden Semester ohne Abmeldung hat. Wenn ja
    // Semesterbeginn zurückgeben, sonst null.
    ...
    List<Kursanmeldung> kursanmeldungen =
        kursanmeldungenTableModel.getKursanmeldungenTableData().getKursanmeldungen();
    for (Kursanmeldung kursanmeldung1 : kursanmeldungen) {
      if (kursanmeldung1.getKurs().getSemester().getSchuljahr().equals(schuljahrPreviousSemester)
          && kursanmeldung1.getKurs().getSemester().getSemesterbezeichnung()
              == semesterbezeichnungPreviousSemester
  ...
  public FindKursCommand.Result findKurs() {
    ...
    FindKursCommand findKursCommand =
        new FindKursCommand(semester, wochentag, zeitBeginn, mitarbeiter);
  ...
  public boolean checkIfSchuelerIsAngemeldet(SchuelerDatenblattModel schuelerDatenblattModel) {
    ...
    return !(anmeldung.getAbmeldedatum() != null
        && !anmeldung.getAbmeldedatum().after(kursanmeldung.getAnmeldedatum()));
  }

  @Override
  public CalculateAnzWochenCommand.Result speichern(
    ...
    kursanmeldungenTableModel
        .getKursanmeldungenTableData()
        .setKursanmeldungen(schuelerDatenblattModel.getSchueler().getSortedKursanmeldungen());
