...
final class SchuelerSuchenModelImpl extends PersonModelImpl implements SchuelerSuchenModel {
  ...
  public String getGeburtsdatumSuchperiode() {
    if ((geburtsdatumSuchperiodeDateFormatString == null)
      ...
      return asString(getGeburtsdatumSuchperiodeBeginn(), geburtsdatumSuchperiodeDateFormatString)
          + " - "
          + asString(getGeburtsdatumSuchperiodeEnde(), geburtsdatumSuchperiodeDateFormatString);
    }
  }

  @Override
  public void setGeburtsdatumSuchperiode(String geburtsdatumSuchperiode)
  ...
  private final CalendarModelAttribute geburtsdatumSuchperiodeBeginnAttribute =
      new CalendarModelAttribute(
          this,
          Field.GEBURTSDATUM_SUCHPERIODE,
  ...
  public void setGeburtsdatumSuchperiodeBeginn(String geburtsdatumSuchperiodeBeginn)
      throws SvmValidationException {
    geburtsdatumSuchperiodeBeginnAttribute.setNewValue(
        false,
        geburtsdatumSuchperiodeBeginn,
        geburtsdatumSuchperiodeDateFormatString,
        isBulkUpdate());
  }

  private final CalendarModelAttribute geburtsdatumSuchperiodeEndeAttribute =
      new CalendarModelAttribute(
          this,
          Field.GEBURTSDATUM_SUCHPERIODE,
          new GregorianCalendar(1980, Calendar.JANUARY, 1),
  ...
  public void setGeburtsdatumSuchperiodeEnde(String geburtsdatumSuchperiodeEnde)
      throws SvmValidationException {
    geburtsdatumSuchperiodeEndeAttribute.setNewValue(
        false,
        geburtsdatumSuchperiodeEnde,
        geburtsdatumSuchperiodeDateFormatString,
        isBulkUpdate());
  }
  ...
  private final CalendarModelAttribute stichtagModelAttribute =
      new CalendarModelAttribute(
          this,
          Field.STICHTAG,
  ...
  public void setKursFuerSucheBeruecksichtigen(boolean isSelected) {
    ...
    firePropertyChange(
        Field.KURS_FUER_SUCHE_BERUECKSICHTIGEN, oldValue, kursFuerSucheBeruecksichtigen);
  }
  ...
  private final StringModelAttribute rollenModelAttribute =
  ...
  private final IntegerModelAttribute kuchenVorstellungModelAttribute =
      new IntegerModelAttribute(
          this,
          Field.KUCHEN_VORSTELLUNG,
          1,
          9,
          new AttributeAccessor<>() {
            @Override
  ...
  public void setKuchenVorstellung(String kuchenVorstellung) throws SvmValidationException {
    try {
      if (!isBulkUpdate()
          && Integer.parseInt(kuchenVorstellung) > maerchen.getAnzahlVorstellungen()) {
        this.kuchenVorstellung = null;
        invalidate();
        throw new SvmValidationException(
            2032,
            "Das Märchen hat nur " + maerchen.getAnzahlVorstellungen() + " Vorstellungen",
            Field.KUCHEN_VORSTELLUNG);
      }
  ...
  private final StringModelAttribute zusatzattributMaerchenModelAttribute =
      new StringModelAttribute(
          this,
  ...
  @Override
  public void setZusatzattributMaerchen(String zusatzattributMaerchen)
      throws SvmValidationException {
  ...
  public void setMaerchenFuerSucheBeruecksichtigen(boolean isSelected) {
    ...
    firePropertyChange(
        Field.MAERCHEN_FUER_SUCHE_BERUECKSICHTIGEN, oldValue, maerchenFuerSucheBeruecksichtigen);
  }

  @Override
  public boolean searchForSpecificKurs() {
    return (wochentag != null && wochentag != Wochentag.ALLE)
        && zeitBeginn != null
        && (mitarbeiter != null && !mitarbeiter.equals(MITARBEITER_ALLE));
  }
  ...
  public boolean checkIfKurseExist() {
    ...
    FindKurseCommand findKurseCommand =
        new FindKurseCommand(semesterKurs, wochentagFind, zeitBeginn, mitarbeiterFind);
  ...
  public SchuelerSuchenTableData suchen(SvmModel svmModel) {
    ...
    Map<Schueler, List<Kurs>> kurseMapTableData =
        determineKurseMapTableData(
            schuelerList, semesterTableData, wochentag, zeitBeginn, mitarbeiter);
    Maerchen maerchenTableData = determineMaerchenTableData(svmModel, semesterTableData);
    Map<Schueler, Maercheneinteilung> maercheneinteilungenMapTableData =
        determineMaercheneinteilungenMapTableData(schuelerList, maerchenTableData);
    return new SchuelerSuchenTableData(
        schuelerList,
        kurseMapTableData,
        semesterTableData,
        (wochentag == Wochentag.ALLE ? null : wochentag),
        zeitBeginn,
        (mitarbeiter == MITARBEITER_ALLE ? null : mitarbeiter),
        null,
        null,
        maercheneinteilungenMapTableData,
        maerchenTableData,
        (gruppe == Gruppe.ALLE ? null : gruppe),
        (elternmithilfeCode == ELTERNMITHILFE_CODE_ALLE ? null : elternmithilfeCode),
        kursFuerSucheBeruecksichtigen,
        maerchenFuerSucheBeruecksichtigen,
        (rollen != null),
        stichtag,
  ...
  private Semester determineSemesterTableData(SvmModel svmModel) {
    if (kursFuerSucheBeruecksichtigen) {
      ...
      for (Semester semester : semestersAll) {
        if (semester.getSchuljahr().equals(maerchen.getSchuljahr())
            && semester.getSemesterbezeichnung() == Semesterbezeichnung.ERSTES_SEMESTER) {
          return semester;
        }
      }
      ...
      FindSemesterForCalendarCommand findSemesterForCalendarCommand =
          new FindSemesterForCalendarCommand(stichtag, erfassteSemester);
  ...
  private Map<Schueler, List<Kurs>> determineKurseMapTableData(
      List<Schueler> schuelerList,
      Semester semesterTableData,
      Wochentag wochentag,
      Time zeitBeginn,
      Mitarbeiter mitarbeiter) {
    CommandInvoker commandInvoker = getCommandInvoker();
    FindKurseMapSchuelerSemesterCommand findKurseMapSchuelerSemesterCommand =
        new FindKurseMapSchuelerSemesterCommand(
            schuelerList,
            semesterTableData,
            (!kursFuerSucheBeruecksichtigen || wochentag == Wochentag.ALLE ? null : wochentag),
            (!kursFuerSucheBeruecksichtigen ? null : zeitBeginn),
            (!kursFuerSucheBeruecksichtigen || mitarbeiter == MITARBEITER_ALLE
                ? null
                : mitarbeiter),
            stichtag,
            isKeineAbgemeldetenKurseAnzeigen(),
  ...
  private Map<Schueler, Maercheneinteilung> determineMaercheneinteilungenMapTableData(
      List<Schueler> schuelerList, Maerchen maerchen) {
    FindMaercheneinteilungenMapSchuelerSemesterCommand
        findMaercheneinteilungenMapSchuelerSemesterCommand =
            new FindMaercheneinteilungenMapSchuelerSemesterCommand(schuelerList, maerchen);
  ...
  public Semester getSemesterInit(SvmModel svmModel) {
    DetermineSemesterInitCommand determineSemesterInitCommand =
        new DetermineSemesterInitCommand(svmModel, 0);
  ...
  public Maerchen getMaerchenInit(SvmModel svmModel) {
    DetermineMaerchenInitCommand determineMaerchenInitCommand =
        new DetermineMaerchenInitCommand(svmModel);
  ...
  public boolean isCompleted() {
    return !(maerchenFuerSucheBeruecksichtigen
        && isSetKuchenVorstellung()
        && kuchenVorstellung > maerchen.getAnzahlVorstellungen());
  }
