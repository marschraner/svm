 ...
 */
public class MonatsstatistikSchuelerModelImpl extends AbstractModel
    implements MonatsstatistikSchuelerModel {

  private Calendar monatJahr;
  private AnAbmeldungenDispensationenSelected anAbmeldungenDispensationen;

  private final CalendarModelAttribute monatJahrModelAttribute =
  ...
  public void setMonatJahr(String monatJahr) throws SvmValidationException {
    monatJahrModelAttribute.setNewValue(
        true, monatJahr, MONAT_JAHR_DATE_FORMAT_STRING, isBulkUpdate());
  }
  ...
  @Override
  public void setAnAbmeldungenDispensationen(
      AnAbmeldungenDispensationenSelected anAbmeldungenDispensationen) {
    ...
    firePropertyChange(
        Field.AN_ABMELDUNGEN_DISPENSATIONEN, oldValue, this.anAbmeldungenDispensationen);
  }

  @Override
  public SchuelerSuchenTableData suchen(SvmModel svmModel) {
    MonatsstatistikSchuelerSuchenCommand monatsstatistikSchuelerSuchenCommand =
        new MonatsstatistikSchuelerSuchenCommand(this);
    ...
    Map<Schueler, List<Kurs>> kurseMapTableData =
        determineKurseMapTableData(schuelerList, semesterTableData);
    ...
    return new SchuelerSuchenTableData(
        ...
        null,
        null,
        (anAbmeldungenDispensationen == AnAbmeldungenDispensationenSelected.ANMELDUNGEN_KURSE
            ? monatJahr
            : null),
        (anAbmeldungenDispensationen == AnAbmeldungenDispensationenSelected.ABMELDUNGEN_KURSE
            ? monatJahr
            : null),
  ...
  private Semester determineSemesterTableData(SvmModel svmModel) {
    ...
    int day;
    // Bei Abmeldungen nicht letzten, sondern ersten Tag des Monats nehmen, damit altes Semester
    // angezeigt wird
    if (anAbmeldungenDispensationen == AnAbmeldungenDispensationenSelected.ABMELDUNGEN_KINDERTHEATER
        || anAbmeldungenDispensationen == AnAbmeldungenDispensationenSelected.ABMELDUNGEN_KURSE) {
      day = 1;
    } else {
      day = monatJahr.getActualMaximum(Calendar.DAY_OF_MONTH);
    }
    Calendar firstOrLastDayOfMonth =
        new GregorianCalendar(monatJahr.get(Calendar.YEAR), monatJahr.get(Calendar.MONTH), day);
    FindSemesterForCalendarCommand findSemesterForCalendarCommand =
        new FindSemesterForCalendarCommand(firstOrLastDayOfMonth, erfassteSemester);
    ...
    // Wenn in Ferien zwischen 2 Semestern vorhergehendes Semester falls Abmeldungen, sonst
    // Folgesemester
    if (currentSemester == null) {
      if (anAbmeldungenDispensationen
              == AnAbmeldungenDispensationenSelected.ABMELDUNGEN_KINDERTHEATER
          || anAbmeldungenDispensationen == AnAbmeldungenDispensationenSelected.ABMELDUNGEN_KURSE) {
  ...
  private Map<Schueler, List<Kurs>> determineKurseMapTableData(
      List<Schueler> schuelerList, Semester semester) {
    CommandInvoker commandInvoker = getCommandInvoker();
