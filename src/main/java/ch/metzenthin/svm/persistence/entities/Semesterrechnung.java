...
public class Semesterrechnung extends AbstractEntity implements Comparable<Semesterrechnung> {
  ...
  public int compareTo(Semesterrechnung otherSemesterrechnung) {
    ...
    List<Schueler> angemeldeteSchuelerRechnungsempfaenger =
        this.rechnungsempfaenger.getAngemeldeteSchuelerRechnungsempfaenger();
    Person person =
        (angemeldeteSchuelerRechnungsempfaenger.isEmpty()
            ? this.rechnungsempfaenger
            : angemeldeteSchuelerRechnungsempfaenger.get(0));
    List<Schueler> angemeldeteSchuelerOtherRechnungsempfaenger =
        otherSemesterrechnung.rechnungsempfaenger.getAngemeldeteSchuelerRechnungsempfaenger();
    Person otherPerson =
        angemeldeteSchuelerOtherRechnungsempfaenger.isEmpty()
            ? otherSemesterrechnung.rechnungsempfaenger
            : angemeldeteSchuelerOtherRechnungsempfaenger.get(0);
  ...
  public BigDecimal getErmaessigungStipendiumVorrechnung() {
    ...
    if (stipendium != null && stipendium != Stipendium.KEINES) {
      ...
      ermaessigungStipendiumVorrechnung =
          ermaessigungStipendiumVorrechnung.multiply(
              BigDecimal.valueOf(1 - stipendium.getFaktor()));
      ermaessigungStipendiumVorrechnung =
          ermaessigungStipendiumVorrechnung.setScale(
              1, RoundingMode.HALF_EVEN); // Runden auf 10 Rappen
  ...
  public BigDecimal getErmaessigungStipendiumNachrechnung() {
    ...
    if (stipendium != null && stipendium != Stipendium.KEINES) {
      ...
      ermaessigungStipendiumNachrechnung =
          ermaessigungStipendiumNachrechnung.multiply(
              BigDecimal.valueOf(1 - stipendium.getFaktor()));
      ermaessigungStipendiumNachrechnung =
          ermaessigungStipendiumNachrechnung.setScale(
              1, RoundingMode.HALF_EVEN); // Runden auf 10 Rappen
  ...
  public BigDecimal getDifferenzSchulgeld() {
    boolean showVorrechnung =
        rechnungsdatumVorrechnung != null
            || (wochenbetragVorrechnung != null
                && wochenbetragVorrechnung.compareTo(BigDecimal.ZERO) != 0)
            || (ermaessigungVorrechnung != null
                && ermaessigungVorrechnung.compareTo(BigDecimal.ZERO) != 0)
  ...
  public boolean isNullrechnung() {
    return (ermaessigungVorrechnung == null
            || ermaessigungVorrechnung.compareTo(BigDecimal.ZERO) == 0)
        && (zuschlagVorrechnung == null || zuschlagVorrechnung.compareTo(BigDecimal.ZERO) == 0)
        && (wochenbetragVorrechnung == null
            || wochenbetragVorrechnung.compareTo(BigDecimal.ZERO) == 0)
        && (betragZahlung1Vorrechnung == null
            || betragZahlung1Vorrechnung.compareTo(BigDecimal.ZERO) == 0)
        && (betragZahlung2Vorrechnung == null
            || betragZahlung2Vorrechnung.compareTo(BigDecimal.ZERO) == 0)
        && (betragZahlung3Vorrechnung == null
            || betragZahlung3Vorrechnung.compareTo(BigDecimal.ZERO) == 0)
        && (ermaessigungNachrechnung == null
            || ermaessigungNachrechnung.compareTo(BigDecimal.ZERO) == 0)
        && (zuschlagNachrechnung == null || zuschlagNachrechnung.compareTo(BigDecimal.ZERO) == 0)
        && (wochenbetragNachrechnung == null
            || wochenbetragNachrechnung.compareTo(BigDecimal.ZERO) == 0)
        && (betragZahlung1Nachrechnung == null
            || betragZahlung1Nachrechnung.compareTo(BigDecimal.ZERO) == 0)
        && (betragZahlung2Nachrechnung == null
            || betragZahlung2Nachrechnung.compareTo(BigDecimal.ZERO) == 0)
        && (betragZahlung3Nachrechnung == null
            || betragZahlung3Nachrechnung.compareTo(BigDecimal.ZERO) == 0);
  }
  ...
  public List<Schueler> getAktiveSchuelerRechnungsempfaenger(Semester previousSemester) {
    List<Schueler> schuelersRechnungsempfaenger =
        new ArrayList<>(rechnungsempfaenger.getSchuelerRechnungsempfaenger());
            ...
            || (previousSemester != null
                ...
                && (kursanmeldung.getAbmeldedatum() == null
                    ...
                    || (kursanmeldung.getAbmeldedatum() != null
                        ...
                        && ((ermaessigungVorrechnung != null
                                && ermaessigungVorrechnung.compareTo(BigDecimal.ZERO) != 0)
                            || (zuschlagVorrechnung != null
                                && zuschlagVorrechnung.compareTo(BigDecimal.ZERO) != 0)))))) {
  ...
  public String getAktiveSchuelerRechnungsempfaengerAsStr(Semester previousSemester) {
    ...
    // Letztes ", " löschen
    if (aktiveSchuelerSb.length() >= 2
        && aktiveSchuelerSb.substring(aktiveSchuelerSb.length() - 2).equals(", ")) {
