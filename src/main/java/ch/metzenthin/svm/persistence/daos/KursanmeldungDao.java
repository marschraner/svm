...
public class KursanmeldungDao extends GenericDao<Kursanmeldung, KursanmeldungId> {
  ...
  public List<Kursanmeldung> findKursanmeldungenSchueler(Schueler schueler) {
    TypedQuery<Kursanmeldung> typedQuery =
        db.getCurrentEntityManager()
            .createQuery(
                "select k from Kursanmeldung k where k.schueler.personId = :personId",
                Kursanmeldung.class);
  ...
  public List<Kursanmeldung> findKursanmeldungen(
      Schueler schueler,
      Semester semester,
      Wochentag wochentag,
      Time zeitBeginn,
      Mitarbeiter mitarbeiter,
      Calendar anmeldemonat,
      Calendar abmeldemonat,
      boolean keineAbgemeldetenKurseAnzeigen,
      Calendar stichtagSchuelerSuchen) {
    ...
    if (anmeldemonat != null) {
      selectStatementSb.append(
          " k.anmeldedatum >= :anmeldemonatBeginn and k.anmeldedatum <= :anmeldemonatEnde and");
    }
    if (abmeldemonat != null) {
      selectStatementSb.append(
          " k.abmeldedatum >= :abmeldemonatBeginn and k.abmeldedatum <= :abmeldemonatEnde and");
    }
    if (keineAbgemeldetenKurseAnzeigen && stichtagSchuelerSuchen != null) {
      selectStatementSb.append(
          " (k.abmeldedatum is null or k.abmeldedatum > :stichtagSchuelerSuchen) and");
    }
    // Letztes " and" löschen
    ...
    // Query
    TypedQuery<Kursanmeldung> typedQuery =
        db.getCurrentEntityManager().createQuery(selectStatementSb.toString(), Kursanmeldung.class);
    if (schueler != null) {
