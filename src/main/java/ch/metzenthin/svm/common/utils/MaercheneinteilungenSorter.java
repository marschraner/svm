...
public class MaercheneinteilungenSorter {
  ...
  public Map<Schueler, Maercheneinteilung> sortMaercheneinteilungenByGruppeAndRolle(
      Map<Schueler, Maercheneinteilung> maercheneinteilungen) {
    List<Map.Entry<Schueler, Maercheneinteilung>> list =
        new LinkedList<>(maercheneinteilungen.entrySet());
    list.sort(
        (entry1, entry2) -> {
          ...
          if (result == 0) {
            ...
            result =
                stringNumberComparator.compare(
                    entry1.getValue().getRolle1(), entry2.getValue().getRolle1());
          }
          return result;
        });
