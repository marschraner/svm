...
public class RechnungsdatumErfassenController extends AbstractController {
  ...
  public RechnungsdatumErfassenController(
      SemesterrechnungenTableModel semesterrechnungenTableModel,
      RechnungsdatumErfassenModel rechnungsdatumErfassenModel,
    ...
    this.rechnungsdatumErfassenModel.addCompletedListener(
        this::onRechnungsdatumErfassenModelCompleted);
  ...
  public void setRechnungsdatumErfassenDialog(JDialog rechnungsdatumErfassenDialog) {
    ...
    rechnungsdatumErfassenDialog.addWindowListener(
        new WindowAdapter() {
          @Override
  ...
  public void setTxtRechnungsdatum(JTextField txtRechnungsdatum) {
    ...
    this.txtRechnungsdatum.addFocusListener(
        new FocusAdapter() {
          @Override
  ...
  private void onRechnungsdatumEvent(boolean showRequiredErrMsg) {
    ...
    boolean equalFieldAndModelValue =
        equalsNullSafe(
            txtRechnungsdatum.getText(), rechnungsdatumErfassenModel.getRechnungsdatum());
    try {
  ...
  private void setModelRechnungsdatum(boolean showRequiredErrMsg) throws SvmValidationException {
    ...
    try {
      ...
      LOGGER.trace(
          "RechnungsdatumErfassenController setModelRechnungsdatum RequiredException={}",
          e.getMessage());
      ...
      LOGGER.trace(
          "RechnungsdatumErfassenController setModelRechnungsdatum Exception={}", e.getMessage());
      showErrMsg(e);
      throw e;
    }
  }
    ...
    if (n == 0) {
      ...
      String infoMessage =
          switch (rechnungstyp) {
            ...
            case NACHRECHNUNG ->
                "Das Rechnungsdatum wird gesetzt und Zahlungen werden kopiert. Bitte warten ...";
          };
      ...
      SwingWorker<Void, Void> worker =
          new SwingWorker<>() {
            ...
            protected Void doInBackground() {
              try {
                // Der Swing-Worker erzeugt einen eigenen Entity-Manager. Die selektierten
                // Semesterrechnungen,
                ...
                semesterrechnungenTableModel
                    .loadSelektierteSemesterrechnungenNotContainedInPersistenceContext();
                rechnungsdatumErfassenModel.replaceRechnungsdatumAndUpdateSemesterrechnung(
                    semesterrechnungenTableModel, rechnungstyp);
              } catch (Exception e) {
            ...
            protected void done() {
              ...
              // Die durch den Swing-Worker veränderten Semesterrechnungen befinden sich nach dem
              // Schliessen der
              // DB-Session des Swing-Workers nicht mehr in dessen Persistenzkontext, welcher
              // mitgelöscht wird.
              // Sie müssen in den Persistenzkontext des ab nun wieder verantwortlichen Main-Threads
              // geladen
              // werden.
              semesterrechnungenTableModel
                  .reloadSemesterrechnungenNotContainedInPersistenceContext();
            }
          };
      worker.execute();
