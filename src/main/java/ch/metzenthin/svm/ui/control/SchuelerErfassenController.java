...
public class SchuelerErfassenController extends AbstractController {
  ...
  public SchuelerErfassenController(
  ...
  public void setMutterPanel(AngehoerigerPanel mutterPanel, AngehoerigerModel mutterModel) {
    ...
    // Anrede auf Frau setzen (Anrede anzeigen für Möglichkeit zur Erfassung gleichgeschlechtlicher
    // Paare)
    try {
  ...
  public void setVaterPanel(AngehoerigerPanel vaterPanel, AngehoerigerModel vaterModel) {
    ...
    // Anrede auf Herr setzen (Anrede anzeigen für Möglichkeit zur Erfassung gleichgeschlechtlicher
    // Paare)
    try {
  ...
  public void setDrittempfaengerPanel(
      AngehoerigerPanel drittempfaengerPanel, AngehoerigerModel drittempfaengerModel) {
    drittempfaengerController =
        drittempfaengerPanel.setModel(drittempfaengerModel, defaultButtonEnabled);
  ...
  private void onAbbrechen() {
    ...
    if (!isBearbeiten) {
      dialogText =
          "Durch Drücken des Ja-Buttons wird die Eingabemaske geschlossen. "
    ...
    if (n == 0) {
      if (!isBearbeiten) {
        ...
        zurueckZuDatenblattListener.actionPerformed(
            new ActionEvent(btnAbbrechen, ActionEvent.ACTION_PERFORMED, "Zurück nach Abbrechen"));
      }
    }
  }
  ...
  private void onSpeichern() {
    ...
    // Wenn isAbbrechen() zurück zur Eingabemaske, sonst Listener aufrufen (wenn vorhanden), der ein
    // neues Panel aufruft
    if (schuelerErfassenSaveResult != null
        && schuelerErfassenSaveResult.getResult()
            == ValidateSchuelerCommand.Result.SPEICHERUNG_ERFOLGREICH) {
      if (zurueckZuDatenblattListener != null) {
        zurueckZuDatenblattListener.actionPerformed(
            new ActionEvent(btnSpeichern, ActionEvent.ACTION_PERFORMED, "Speichern erfolgreich"));
      } else {
        SchuelerSuchenTableData schuelerSuchenTableData =
            schuelerErfassenModel.getSchuelerSuchenTableData();
        ...
        SchuelerDatenblattPanel schuelerDatenblattPanel =
            new SchuelerDatenblattPanel(svmContext, schuelerSuchenTableModel, null, 0, false);
    ...
    if (n == 0) {
      ...
      switch (result) {
        ...
        case LOESCHEN_ERFOLGREICH -> {
          ...
          closeListener.actionPerformed(
              new ActionEvent(btnAbbrechen, ActionEvent.ACTION_PERFORMED, "Close nach Löschen"));
        }
      }
    } else {
      btnSchuelerLoeschen.setFocusPainted(false);
    ...
    if (n == 0) {
      ...
      zurueckZuDatenblattListener.actionPerformed(
          new ActionEvent(
              btnSpeichern,
              ActionEvent.ACTION_PERFORMED,
              "Frühere Anmeldungen erfolgreich gelöscht"));
    } else {
      btnFruehereAnmeldungenLoeschen.setFocusPainted(false);
    }
  }

  private SchuelerErfassenDialog createDialog(
      SchuelerErfassenSaveResult schuelerErfassenSaveResult) {
    final SchuelerErfassenDialog[] dialog =
        new SchuelerErfassenDialog[1]; // Array, weil Variable final sein muss
    schuelerErfassenSaveResult.accept(
        new SchuelerErfassenSaveResultVisitor() {
          @Override
          public void visit(ValidateSchuelerSummaryResult validateSchuelerSummaryResult) {
            dialog[0] =
                new ValidateSchuelerSummaryDialog(
                    validateSchuelerSummaryResult, schuelerErfassenModel);
          }
          ...
          public void visit(
            ...
            dialog[0] =
                new AngehoerigerMehrereEintraegePassenDialog(
                    angehoerigerMehrereEintraegePassenResult, schuelerErfassenModel);
          }
          ...
          public void visit(
              AngehoerigerMehrereEintraegeGleicherNameAndereAttributeResult
                  angehoerigerMehrereEintraegeGleicherNameAndereAttributeResult) {
            dialog[0] =
                new AngehoerigerMehrereEintraegeGleicherNameAndereAttributeDialog(
                    angehoerigerMehrereEintraegeGleicherNameAndereAttributeResult,
                    schuelerErfassenModel);
          }

          @Override
          public void visit(
              AngehoerigerEinEintragGleicherNameAndereAttributeResult
                  angehoerigerEinEintragGleicherNameAndereAttributeResult) {
            dialog[0] =
                new AngehoerigerEinEintragGleicherNameAndereAttributeDialog(
                    angehoerigerEinEintragGleicherNameAndereAttributeResult, schuelerErfassenModel);
          }

          @Override
          public void visit(AngehoerigerEinEintragPasstResult angehoerigerEinEintragPasstResult) {
            dialog[0] =
                new AngehoerigerEinEintragPasstDialog(
                    angehoerigerEinEintragPasstResult, schuelerErfassenModel);
          }
          ...
          @Override
          public void visit(
              SchuelerErfassenUnerwarteterFehlerResult schuelerErfassenUnerwarteterFehlerResult) {
          ...
          @Override
          public void visit(
              DrittpersonIdentischMitElternteilResult drittpersonIdentischMitElternteilResult) {
            JOptionPane.showMessageDialog(
                null,
