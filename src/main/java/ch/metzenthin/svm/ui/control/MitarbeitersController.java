...
public class MitarbeitersController {
  ...
  public void setMitarbeitersTable(final JTable mitarbeitersTable) {
    ...
    mitarbeitersTable.addMouseListener(
        new MouseAdapter() {
          @Override
  ...
  private void setLblTotal() {
    String lblTotalText =
        "Total: "
            + mitarbeitersTableModel.getRowCount()
            + " Mitarbeiter ("
            + mitarbeitersTableModel.getAnzSelektiert()
            + " selektiert)";
  ...
  private void onBearbeiten() {
    ...
    Mitarbeiter selectedMitarbeiter =
        mitarbeitersTableModel
            .getMitarbeiters()
            .get(mitarbeitersTable.convertRowIndexToModel(mitarbeitersTable.getSelectedRow()));
    CreateOrUpdateMitarbeiterModel createOrUpdateMitarbeiterModel =
        svmContext
            .getModelFactory()
  ...
  private void onEmail() {
    ...
    if (anzSelektiert > 1 && anzSelektiert < rowCount) {
      JOptionPane.showMessageDialog(
          ...
          "Es sind nur "
              ...
              + " der "
              + rowCount
              + " Einträge selektiert. Nur diese Einträge\nwerden"
              + " für die Gruppen-E-Mail berücksichtigt.",
          "Nicht alle Einträge selektiert",
          JOptionPane.INFORMATION_MESSAGE);
    }
    CallDefaultEmailClientCommand.Result result =
        mitarbeitersModel.callEmailClient(mitarbeitersTableModel);
    ...
    if (!mitarbeitersModel.getFehlendeEmailAdressen().isEmpty()) {
      ...
      JOptionPane.showMessageDialog(
          null,
          "Für folgende Mitarbeiter ist keine E-Mail-Adresse erfasst:\n" + fehlend,
          "Warnung",
          JOptionPane.WARNING_MESSAGE);
    }
