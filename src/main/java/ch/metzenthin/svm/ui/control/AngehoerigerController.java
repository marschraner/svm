...
public class AngehoerigerController extends PersonController {
  ...
  public void onGleicheAdresseWieSchuelerEvent() {
    LOGGER.trace(
        "AngehoerigerController Event GleicheAdresseWieSchueler. Selected={}",
        checkBoxGleicheAdresseWieSchueler.isSelected());
    setModelGleicheAdresseWieSchueler();
  }

  private void setModelGleicheAdresseWieSchueler() {
    angehoerigerModel.setIsGleicheAdresseWieSchueler(
        checkBoxGleicheAdresseWieSchueler.isSelected());
  }
  ...
  private void onWuenschtEmailsEvent() {
    LOGGER.trace(
        "AngehoerigerController Event WuenschtEmails. Selected={}",
        checkBoxWuenschtEmails.isSelected());
  ...
  private void onRechnungsempfaengerEvent() {
    LOGGER.trace(
        "AngehoerigerController Event Rechnungsempfaenger. Selected={}",
  ...
  void doPropertyChange(PropertyChangeEvent evt) {
    LOGGER.trace(
        "AngehoerigerController PropertyChangeEvent '{}', oldValue='{}', newValue='{}'",
        evt.getPropertyName(),
        evt.getOldValue(),
        evt.getNewValue());
    if (checkIsFieldChange(Field.GLEICHE_ADRESSE_WIE_SCHUELER, evt)) {
      checkBoxGleicheAdresseWieSchueler.setSelected(
          angehoerigerModel.isGleicheAdresseWieSchueler());
    } else if (checkIsFieldChange(Field.WUENSCHT_EMAILS, evt)
        && checkBoxWuenschtEmails.isVisible()) {
      checkBoxWuenschtEmails.setSelected(
          angehoerigerModel.getWuenschtEmails() != null && angehoerigerModel.getWuenschtEmails());
  ...
  public void makeErrorLabelsInvisible(Set<Field> fields) {
    ...
    if (fields.contains(Field.WUENSCHT_EMAILS) && checkBoxWuenschtEmails.isVisible()) {
