...
public class KursanmeldungenController {
  ...
  public void setKursanmeldungenTable(JTable kursanmeldungenTable) {
    ...
    kursanmeldungenTable.addMouseListener(
        new MouseAdapter() {
          @Override
  ...
  private void onNeu() {
    ...
    KursanmeldungErfassenDialog kursanmeldungErfassenDialog =
        new KursanmeldungErfassenDialog(
            svmContext,
            kursanmeldungenModel,
            kursanmeldungenTableModel,
            schuelerDatenblattModel,
            kursanmeldungenTable.getSelectedRow(),
            false,
            "Kursanmeldung erfassen");
  ...
  private void onBearbeiten() {
    ...
    KursanmeldungErfassenDialog kursanmeldungErfassenDialog =
        new KursanmeldungErfassenDialog(
            ...
            kursanmeldungenTable.getSelectedRow(),
            true,
            "Kursanmeldung bearbeiten");
    kursanmeldungErfassenDialog.pack();
    kursanmeldungErfassenDialog.setVisible(true);
    kursanmeldungenTableModel.fireTableDataChanged();
    btnBearbeiten.setFocusPainted(false);

    // Falls sich der Schüler von sämtlichen Kursen des neusten Semesters abgemeldet hat, soll er
    // wahrscheinlich auch vom Theater abgemeldet werden
    Calendar spaetestesAbmeldedatumNeustesSemester =
        kursanmeldungenModel.getSpaetestesAbmeldedatumKurseNeustesSemester(schuelerDatenblattModel);
    if (schuelerDatenblattModel.getSchueler().isAngemeldet()
        && spaetestesAbmeldedatumNeustesSemester != null) {
      ...
      int n =
          JOptionPane.showOptionDialog(
              ...
              "Soll "
                  ...
                  + " per "
                  + DateAndTimeUtils.getCalendarAsDDMMYYYY(spaetestesAbmeldedatumNeustesSemester)
                  + " vom Kinder- und Jugendtheater abgemeldet werden?",
      ...
      if (n == 0) {
        kursanmeldungenModel.schuelerVomKinderUndJugendtheaterAbmelden(
            schuelerDatenblattModel, spaetestesAbmeldedatumNeustesSemester);
        JOptionPane.showMessageDialog(
            null,
            "Der Schüler wurde per "
                + DateAndTimeUtils.getCalendarAsDDMMYYYY(spaetestesAbmeldedatumNeustesSemester)
                + " abgemeldet.",
  ...
  private void onLoeschen() {
    ...
    if (n == 0) {
      kursanmeldungenModel.kursanmeldungLoeschen(
          kursanmeldungenTableModel,
          schuelerDatenblattModel,
          kursanmeldungenTable.getSelectedRow());
      kursanmeldungenTableModel.fireTableDataChanged();
