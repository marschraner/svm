...
import ch.metzenthin.svm.domain.model.ListenExportModelOld;
...
public class ListenExportControllerOld extends AbstractController {

  private static final Logger LOGGER = LoggerFactory.getLogger(ListenExportControllerOld.class);

  // Möglichkeit zum Umschalten des validation modes (nicht dynamisch)
  private static final boolean MODEL_VALIDATION_MODE = false;

  private final ListenExportModelOld listenExportModelOld;
  ...
  public ListenExportControllerOld(
      ListenExportModelOld listenExportModelOld,
    ...
    super(listenExportModelOld);
    this.listenExportModelOld = listenExportModelOld;
    ...
    this.listenExportModelOld.addPropertyChangeListener(this);
    this.listenExportModelOld.addDisableFieldsListener(this);
    this.listenExportModelOld.addMakeErrorLabelsInvisibleListener(this);
    this.listenExportModelOld.addCompletedListener(this::onListenExportModelCompleted);
    this.setModelValidationMode(MODEL_VALIDATION_MODE);
  }

  public void constructionDone() {
    listenExportModelOld.initializeCompleted();
  }
  ...
  public void setListenExportDialog(JDialog listenExportDialog) {
    ...
    listenExportDialog.addWindowListener(
        new WindowAdapter() {
          @Override
  ...
  private void onListentypSelected() {
    LOGGER.trace(
        "ListenExportController Event Listentyp selected={}", comboBoxListentyp.getSelectedItem());
    boolean equalFieldAndModelValue =
        equalsNullSafe(comboBoxListentyp.getSelectedItem(), listenExportModelOld.getListentyp());
    try {
      setModelListentyp();
  ...
  private void setModelListentyp() throws SvmRequiredException {
    ...
    try {
      listenExportModelOld.setListentyp((Listentyp) comboBoxListentyp.getSelectedItem());
  ...
  private void initComboBoxListentyp() {
    if (listenExportTyp == ListenExportTyp.SCHUELER) {
      if (!schuelerSuchenTableModel.isKursFuerSucheBeruecksichtigen()
          || schuelerSuchenTableModel.getWochentag() == null
      ...
      if (schuelerSuchenTableModel.getMaerchen() == null
          || schuelerSuchenTableModel.getAnzZuExportierendeMaercheneinteilungen() == 0) {
      ...
      if (schuelerSuchenTableModel.isMaerchenFuerSucheBeruecksichtigen()
          && !schuelerSuchenTableModel.isKursFuerSucheBeruecksichtigen()) {
        comboBoxListentyp.removeItem(Listentyp.SCHUELER_ADRESSLISTE);
        comboBoxListentyp.removeItem(Listentyp.RECHNUNGSEMPFAENGER_ADRESSETIKETTEN);
  ...
  public void setTxtTitel(JTextField txtTitel) {
    ...
    this.txtTitel.addFocusListener(
        new FocusAdapter() {
          @Override
  ...
  private void initTitel() {
    ...
    if (txtTitel == null || listenExportModelOld.getListentyp() == null) {
      return;
    }
    String titleInit = listenExportModelOld.getTitleInit(schuelerSuchenTableModel);
  ...
  private void onTitelEvent(boolean showRequiredErrMsg) {
    ...
    boolean equalFieldAndModelValue =
        equalsNullSafe(txtTitel.getText(), listenExportModelOld.getTitel());
    try {
  ...
  private void setModelTitel(boolean showRequiredErrMsg) throws SvmValidationException {
    ...
    try {
      listenExportModelOld.setTitel(txtTitel.getText());
    } catch (SvmRequiredException e) {
  ...
  private void onOk() {
    ...
    enableButtons(false);
    // Vorprüfung
    String[] listenErstellenWarning =
        listenExportModelOld.getListenErstellenWarning(semesterrechnungenTableModel);
    ...
    final JOptionPane optionPane =
        new JOptionPane(
            "Die Datei wird erstellt. Bitte warten ...",
    ...
    SwingWorker<CreateListeCommand.Result, String> worker =
        new SwingWorker<>() {
          @Override
          protected CreateListeCommand.Result doInBackground() {
            return listenExportModelOld.createListenFile(
                outputFile,
              ...
              switch (result) {
                case TEMPLATE_FILE_EXISTIERT_NICHT_ODER_NICHT_LESBAR ->
                    JOptionPane.showMessageDialog(
                        listenExportDialog,
                        "Template-Datei '"
                            + listenExportModelOld.getTemplateFile()
                            + "' nicht gefunden."
  ...
  private JFileChooser setupFileChooser() {
    ...
    if (listenExportModelOld.getSaveFileInit() != null) {
      fileChooser.setSelectedFile(listenExportModelOld.getSaveFileInit());
    }
    final Filetyp filetyp = listenExportModelOld.getListentyp().getFiletyp();
  ...
  private void enableDisableFields() {
    if (listenExportModelOld.getListentyp() != null
        && listenExportModelOld.getListentyp().getFiletyp() == Filetyp.CSV) {
      disableTitel();
    } else {
      enableTitel();
    }
  ...
  private void enableTitel() {
    listenExportModelOld.enableFields(getTitelField());
  }

  private void disableTitel() {
    listenExportModelOld.disableFields(getTitelField());
    listenExportModelOld.makeErrorLabelsInvisible(getTitelField());
  }
  ...
  void doPropertyChange(PropertyChangeEvent evt) {
    ...
    if (checkIsFieldChange(Field.LISTENTYP, evt)) {
      comboBoxListentyp.setSelectedItem(listenExportModelOld.getListentyp());
      initTitel();
    } else if (checkIsFieldChange(Field.TITEL, evt)) {
      txtTitel.setText(listenExportModelOld.getTitel());
    }
  }
  ...
  public void disableFields(boolean disable, Set<Field> fields) {
    if (comboBoxListentyp != null
        && (fields.contains(Field.ALLE) || fields.contains(Field.LISTENTYP))) {
