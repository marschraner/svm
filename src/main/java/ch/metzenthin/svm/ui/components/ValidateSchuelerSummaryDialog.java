...
public class ValidateSchuelerSummaryDialog extends SchuelerErfassenDialog {
  ...
  private void setSchueler() {
    lblSchueler.setText(
        validateSchuelerSummaryResult.getSchueler().getGeschlecht() == Geschlecht.W
            ? "Schülerin:"
            : "Schüler:");
  ...
  private void setMutter() {
    String neuMutter =
        ((validateSchuelerSummaryResult.getMutter() != null
                && validateSchuelerSummaryResult.isMutterNeu())
            ? " (neu)"
            : "");
  ...
  private void setVater() {
    String neuVater =
        ((validateSchuelerSummaryResult.getVater() != null
                && validateSchuelerSummaryResult.isVaterNeu())
            ? " (neu)"
            : "");
  ...
  private void setRechnungsempfaenger() {
    ...
    if (validateSchuelerSummaryResult.isRechnungsempfaengerMutter()) {
      ...
      String neuRechnungsempfaenger =
          (validateSchuelerSummaryResult.isRechnungsempfaengerNeu() ? " (neu)" : "");
      lblRechnungsempfaengerText =
          (validateSchuelerSummaryResult.getRechnungsempfaenger().getAnrede() == Anrede.FRAU
              ? "Rechnungsempfängerin" + neuRechnungsempfaenger + ":"
              : "Rechnungsempfänger" + neuRechnungsempfaenger + ":");
  ...
  private void setGeburtsdatum() {
    geburtsdatumValue.setText(
        asString(validateSchuelerSummaryResult.getSchueler().getGeburtsdatum()));
  }

  private void setAnmeldedatum() {
    anmeldedatumValue.setText(
        asString(
            validateSchuelerSummaryResult.getSchueler().getAnmeldungen().get(0).getAnmeldedatum()));
  }

  private void setAbmeldedatum() {
    if (validateSchuelerSummaryResult.getSchueler().getAnmeldungen().get(0).getAbmeldedatum()
        == null) {
      abmeldedatum.setVisible(false);
      abmeldedatumValue.setVisible(false);
    } else {
      abmeldedatumValue.setText(
          asString(
              validateSchuelerSummaryResult
                  .getSchueler()
                  .getAnmeldungen()
  ...
  private void setInfoIdentischeAdressen() {
    ...
    if (identischeAdressen != null
        && !identischeAdressen.isEmpty()
        && abweichendeAdressen != null
        && !abweichendeAdressen.isEmpty()) {
  ...
  private void setGeschwister() {
    if (validateSchuelerSummaryResult.getGeschwister() != null
        && !validateSchuelerSummaryResult.getGeschwister().isEmpty()) {
  ...
  private void setSchuelerGleicherRechnungsempfaenger() {
    ...
    if (validateSchuelerSummaryResult.getAndereSchueler() != null
        && !validateSchuelerSummaryResult.getAndereSchueler().isEmpty()) {
      ...
      schuelerGleicherRechnungsempfaengerValue.setText(
          schuelerGleicherRechnungsempfaengerStb.toString());
    } else {
      schuelerGleicherRechnungsempfaengerValue.setText("-");
    }
  }
...
// DO NOT EDIT OR ADD ANY CODE HERE!
  ...
  private void $$$setupUI$$$() {
    ...
    panel3.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), "Zusammefassung",
        TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION,
        this.$$$getFont$$$(null, Font.BOLD, -1, panel3.getFont()), new Color(-16777216)));
